/*
    Increase masterwork weapons to give +d6 cold damage.
*/
...
void main()
{
    ...
    if (GetItemHasItemProperty(oWeapon, ITEM_PROPERTY_DAMAGE_BONUS))
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            ...
            {
                if(GetItemPropertySubType(ip) == IP_CONST_DAMAGETYPE_COLD)
                {
                    ...
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_COLD,iNew), oWeapon);
                    iBreak = TRUE;
                }
            }
