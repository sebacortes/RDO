...
void main()
{
       ...
       !GetObjectSeen(oEnemy))  // Nearest enemy is not seen.
    {
        ...
        if(AI_AttemptHealingAlly()){SetLocalInt(OBJECT_SELF,AI_NEEDREST,i1);return;}
        ...
        if(AI_AttemptCureCondition()){SetLocalInt(OBJECT_SELF,AI_NEEDREST,i1);return;}

        // rest part(with force rest)
        if(GetLocalInt(OBJECT_SELF,AI_NEEDREST) && GetSpawnInCondition(AI_FLAG_OTHER_REST_AFTER_COMBAT, AI_OTHER_MASTER) &&
        !GetLocalTimer(AI_TIMER_SEARCH) &&
        (!GetIsObjectValid(oEnemy) || (GetIsObjectValid(oEnemy) && GetDistanceToObject(oEnemy)>f40)))
        {
            // fastbuff reset
            if(GetLocalInt(OBJECT_SELF,AI_NEEDFASTBUFF)==i2)
            {
                SetSpawnInCondition(AI_FLAG_COMBAT_FLAG_FAST_BUFF_ENEMY,AI_COMBAT_MASTER);
                DeleteLocalInt(OBJECT_SELF,AI_NEEDFASTBUFF);
            }
            DeleteLocalInt(OBJECT_SELF,AI_NEEDREST);
           ...
           GetIsObjectValid(oEnemy) && GetDistanceToObject(oEnemy) <= f50)
        {
            ...
            SetLocalInt(OBJECT_SELF,AI_NEEDFASTBUFF,i1);
            return;
        }
    ...
    {
        if(GetLocalInt(OBJECT_SELF,AI_NEEDREST)==i0)
        {
            SetLocalInt(OBJECT_SELF,AI_NEEDREST,i1);
        }
        if(GetLocalInt(OBJECT_SELF,AI_NEEDFASTBUFF)==i1)
        {
            SetLocalInt(OBJECT_SELF,AI_NEEDFASTBUFF,i2);
        }
    }
