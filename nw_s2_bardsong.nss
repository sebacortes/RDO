...
void main()
{
    ...
    // ---> Agregado por dragoncin: instrumentos magicos que afectan la cancion
    object oInstrument = GetItemInSlot(INVENTORY_SLOT_RIGHTHAND);
    object oInstrumentLeftHand = GetItemInSlot(INVENTORY_SLOT_LEFTHAND);
    string resRefInstrumentLeftHand = GetResRef(oInstrumentLeftHand);
    if (GetIsObjectValid(oInstrument)) {
        if (GetItemHasItemProperty(oInstrument, ITEM_PROPERTY_ENHANCEMENT_BONUS))
        {
            itemproperty ipEnhancement = GetFirstItemProperty(oInstrument);
            while (GetIsItemPropertyValid(ipEnhancement) && GetItemPropertyType(ipEnhancement)!=ITEM_PROPERTY_ENHANCEMENT_BONUS)
            {
                ipEnhancement = GetNextItemProperty(oInstrument);
            }
            int nBonus = GetItemPropertyCostTableValue(ipEnhancement);
            //SendMessageToPC(OBJECT_SELF, "nBonus= "+IntToString(nBonus));
            string refRefInstrument = GetResRef(oInstrument);
            if (refRefInstrument == "ips_lute" && nAC > 0)
                nAC += nBonus;
            else if (refRefInstrument == "ips_violinbow" && nAttack > 0)
            {
                if (resRefInstrumentLeftHand == "ips_violinm" || resRefInstrumentLeftHand == "ips_violinf" ||
                    resRefInstrumentLeftHand == "zep_violinm" || resRefInstrumentLeftHand == "zep_violinf" )
                {
                    nAttack += nBonus;
                    nDamage += nBonus;
                }
            }
            else if (refRefInstrument == "ips_harp")
            {
                nFort += (nFort > 0) ? nBonus : 0;
                nWill += (nWill > 0) ? nBonus : 0;
                nReflex += (nReflex > 0) ? nBonus : 0;
            }
            else if (refRefInstrument == "ips_tambourine" && nHP > 0)
                nHP += nBonus * 8;
            else if (refRefInstrument == "ips_pipes" && nSkill > 0)
                nSkill += nBonus;
        }
    }
    // <---
    ...
    // ---> Inspirational Boost
    if (GetHasSpellEffect(SPELL_INSPIRATIONAL_BOOST))
    {
        nAttack++;
        nDamage++;
        nWill++;
    ...
    if (resRefInstrumentLeftHand == "ips_trumpet" && GetItemHasItemProperty(oInstrument, ITEM_PROPERTY_ENHANCEMENT_BONUS))
    {
        itemproperty ipEnhancement = GetFirstItemProperty(oInstrument);
        while (GetIsItemPropertyValid(ipEnhancement) && GetItemPropertyType(ipEnhancement)!=ITEM_PROPERTY_ENHANCEMENT_BONUS)
        {
        ...
        int nBonus = GetItemPropertySubType(ipEnhancement);

        effect eSaveFear = EffectSavingThrowIncrease(SAVING_THROW_WILL, 2*nBonus, SAVING_THROW_TYPE_FEAR);
