...
int PPISUserDefinedAllowAccess(object oPC)
{
    ...
    SendMessageToPC(oPC, "Debes pagar el derecho de usar esto!");
    return(FALSE);
}
...
void PPISStorageHeartbeat(object oPC, object oChest) // oChest debería ser lo mismo que OBJECT_SELF, es al pedo recibirlo pero bueh.
{
    ...
    if( GetIsObjectValid(oPC) ) {
        if( GetIsOpen(oChest) )
            ...
            if( GetLocalInt(oPC, "cambio") && PPISStore(oStorage) ) {
                SetLocalInt( oPC, "isExportPending", TRUE ); // Hace que se exporte el PJ en el proximo heartbeat.
                SendMessageToPC(oPC, IntToString(GetLocalInt(oStorage, PPIS_COUNT))+" items fueron guardados satisfactoriamente");
            }
            else
                SpeakString("Error: no se puede guardar los item de "+GetName(oPC)+" en la base de datos de esta area"+GetName(GetArea(OBJECT_SELF))+".", TALKVOLUME_SHOUT);
        }
    }
    else
        DeleteLocalInt(oStorage, PPIS_HB_MUTEX );
...
void PPISWaitUntilRetrieved(object oStorage, int iTry)
{
    if (GetLocalInt(oStorage, PPIS_STATE) == PPIS_STATE_UNKNOWN)
    {
        if (iTry < 60)
        {
            ...
            SendMessageToPC(OBJECT_SELF, "Recuperando inventario");
            AssignCommand(OBJECT_SELF, PPISWaitUntilRetrieved(oStorage, iTry+1));
        } else {
            SpeakString("PPIS ERROR: no se puede cargar el inventario demasiado LAG", TALKVOLUME_SHOUT);
        }
    } else {
        SendMessageToPC(OBJECT_SELF, "Usted tiene "+IntToString(GetLocalInt(oStorage, PPIS_COUNT))+" items guardados.");
...
void main()
{
    ...
    if ( !GetLocalInt(oStorage, PPIS_HB_MUTEX) )
    {
        object oChest = oStorage; // ¿y esto para que?
        SetLocalInt(oStorage, PPIS_HB_MUTEX, TRUE);
        AssignCommand(oStorage, DelayCommand(PPIS_STORAGE_HEARTBEAT, PPISStorageHeartbeat(oPC, oChest))); // para que mandar oChest si sera igual a OBJECT_SELF
    }
