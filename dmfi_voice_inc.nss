...
const int IDIOMA_COMUN      = 0;
const int IDIOMA_ELFICO     = 1;
const int IDIOMA_GNOMICO    = 2;
const int IDIOMA_MEDIANO    = 3;
const int IDIOMA_ENANO      = 4;
const int IDIOMA_ORCO       = 5;
const int IDIOMA_GOBLIN     = 6;
const int IDIOMA_DRACONICO  = 7;
const int IDIOMA_DRUIDICO   = 8;
const int IDIOMA_LADRON     = 9;
const int IDIOMA_CELESTIAL  = 10;
const int IDIOMA_ABYSSAL    = 11;
const int IDIOMA_INFERNAL   = 12;
const int IDIOMA_DROW       = 13;
const int IDIOMA_AQUAN      = 14;
const int IDIOMA_AURAN      = 15;
const int IDIOMA_IGNAN      = 16;
const int IDIOMA_TERRAN     = 17;
const int IDIOMA_GIGANTE    = 18;
const int IDIOMA_GNOLL      = 19;
const int IDIOMA_SILVANO    = 20;
const int IDIOMA_BAJOCOMUN  = 21;

const string IDIOMA_PREFIJO_RESREF = "hlslang_";

void Idiomas_sendMessageToAllDMs( string message );
void Idiomas_sendMessageToAllDMs( string message )
{
    object oPC = GetFirstPC();
    while ( GetIsObjectValid(oPC) )
    {
        if ( GetIsDM(oPC) ) { SendMessageToPC( oPC, message ); }
        oPC = GetNextPC();
    }
}

// Devuelve si oPC conoce el idioma o no
int Idiomas_pjSabeIdioma( object oPC, int idioma );
int Idiomas_pjSabeIdioma( object oPC, int idioma )
{
    //SendMessageToPC( oPC, "idioma= "+IntToString(idioma) );
    if ( idioma > IDIOMA_COMUN )
    {
        string itemTag = IDIOMA_PREFIJO_RESREF + IntToString( idioma );
        return GetIsObjectValid( GetItemPossessedBy(oPC, itemTag) );
    }
    else
        return TRUE;
}

const string Idiomas_DATABASE                                   = "idiomas";
const string Idiomas_DB_LENGUAJES_NIVEL1_APRENDIDOS             = "IDIOMAS_APRENDIDOS_EN_NIVEL1";
const string Idiomas_DB_LENGUAJES_APRENDIDOS_POR_SKILL          = "IDIOMAS_APRENDIDOS_POR_SKILL";
const string Idiomas_CANTIDAD_LENGUAJES_NIVEL1_PARA_APRENDER    = "LENGUAJES_PARA_APRENDER_NIVEL1";
const string Idiomas_CANTIDAD_LENGUAJES_PARA_APRENDER_POR_SKILL = "LENGUAJES_PARA_APRENDER_POR_SKILL";

// Devuelve TRUE si se abrio un dialogo para aprender un nuevo idioma
int Idiomas_aprenderNuevosIdiomas( object oPC );
int Idiomas_aprenderNuevosIdiomas( object oPC )
{
    int aprendeNuevosIdiomas = FALSE;
    int campaignNivel1 = GetCampaignInt(Idiomas_DATABASE, Idiomas_DB_LENGUAJES_NIVEL1_APRENDIDOS, oPC);
    //SendMessageToPC(oPC, "campaignNivel1: "+IntToString(campaignNivel1));
    int cantidadIdiomasNivel1ParaAprender = (campaignNivel1 <= 0) ? ((GetAbilityScore(oPC, ABILITY_INTELLIGENCE, TRUE) - 10) / 2) : 0;
    //SendMessageToPC(oPC, "cantidadIdiomasNivel1ParaAprender: "+IntToString(cantidadIdiomasNivel1ParaAprender));
    int cantidadIdiomasParaAprenderPorSkill = GetSkillRank(SKILL_LANGUAGE, oPC, TRUE);
    cantidadIdiomasParaAprenderPorSkill -= GetCampaignInt(Idiomas_DATABASE, Idiomas_DB_LENGUAJES_APRENDIDOS_POR_SKILL, oPC);;
    //SendMessageToPC(oPC, "cantidadIdiomasParaAprenderPorSkill: "+IntToString(cantidadIdiomasParaAprenderPorSkill));
    if (cantidadIdiomasNivel1ParaAprender > 0 || cantidadIdiomasParaAprenderPorSkill > 0)
    {
        SetLocalInt(oPC, Idiomas_CANTIDAD_LENGUAJES_NIVEL1_PARA_APRENDER, cantidadIdiomasNivel1ParaAprender);
        SetLocalInt(oPC, Idiomas_CANTIDAD_LENGUAJES_PARA_APRENDER_POR_SKILL, cantidadIdiomasParaAprenderPorSkill);
        AssignCommand(oPC, ActionStartConversation(oPC, "learn_language", TRUE, FALSE));
        aprendeNuevosIdiomas =  TRUE;
    }
    return aprendeNuevosIdiomas;
}

void Idiomas_accionAprenderNuevosIdiomas( object oPC );
void Idiomas_accionAprenderNuevosIdiomas( object oPC )
{
...
const string Idiomas_SHORTCUT_ELFICO     = "ELF";
const string Idiomas_SHORTCUT_GNOMICO    = "GN";
const string Idiomas_SHORTCUT_MEDIANO    = "MED";
const string Idiomas_SHORTCUT_ENANO      = "EN";
const string Idiomas_SHORTCUT_ORCO       = "ORC";
const string Idiomas_SHORTCUT_GOBLIN     = "GOB";
const string Idiomas_SHORTCUT_DRACONICO  = "DRA";
const string Idiomas_SHORTCUT_DRUIDICO   = "DRU";
const string Idiomas_SHORTCUT_LADRON     = "LAD";
const string Idiomas_SHORTCUT_CELESTIAL  = "CEL";
const string Idiomas_SHORTCUT_ABYSSAL    = "ABY";
const string Idiomas_SHORTCUT_INFERNAL   = "INF";
const string Idiomas_SHORTCUT_DROW       = "DROW";
const string Idiomas_SHORTCUT_AQUAN      = "AQU";
const string Idiomas_SHORTCUT_AURAN      = "AUR";
const string Idiomas_SHORTCUT_IGNAN      = "IGN";
const string Idiomas_SHORTCUT_TERRAN     = "TER";
const string Idiomas_SHORTCUT_GIGANTE    = "GIG";
const string Idiomas_SHORTCUT_GNOLL      = "GNO";
const string Idiomas_SHORTCUT_SILVANO    = "SIL";
const string Idiomas_SHORTCUT_BAJOCOMUN  = "UND";

// Devuelve el idioma de la frase en base al shorcut *foobar* que tenga
int Idiomas_getLanguageFromShortcut( string message );
int Idiomas_getLanguageFromShortcut( string message )
{
    int idioma = IDIOMA_COMUN;
    string tmp_msg = message;
    if ( GetStringLeft(message, 1) == "*" ) {
        tmp_msg = GetStringRight( message, GetStringLength(message)-1 );
        if ( FindSubString(tmp_msg, "*") > 0 ) {
            tmp_msg = GetStringLeft( tmp_msg, FindSubString(tmp_msg, "*") );
            tmp_msg = GetStringUpperCase( tmp_msg );
            if ( tmp_msg == Idiomas_SHORTCUT_ELFICO ) {
                idioma = IDIOMA_ELFICO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_GNOMICO ) {
                idioma = IDIOMA_GNOMICO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_MEDIANO ) {
                idioma = IDIOMA_MEDIANO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_ENANO ) {
                idioma = IDIOMA_ENANO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_ORCO ) {
                idioma = IDIOMA_ORCO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_GOBLIN ) {
                idioma = IDIOMA_GOBLIN;
            } else if ( tmp_msg == Idiomas_SHORTCUT_DRACONICO ) {
                idioma = IDIOMA_DRACONICO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_DRUIDICO ) {
                idioma = IDIOMA_DRUIDICO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_LADRON ) {
                idioma = IDIOMA_LADRON;
            } else if ( tmp_msg == Idiomas_SHORTCUT_CELESTIAL ) {
                idioma = IDIOMA_CELESTIAL;
            } else if ( tmp_msg == Idiomas_SHORTCUT_ABYSSAL ) {
                idioma = IDIOMA_ABYSSAL;
            } else if ( tmp_msg == Idiomas_SHORTCUT_INFERNAL ) {
                idioma = IDIOMA_INFERNAL;
            } else if ( tmp_msg == Idiomas_SHORTCUT_DROW ) {
                idioma = IDIOMA_DROW;
            } else if ( tmp_msg == Idiomas_SHORTCUT_AQUAN ) {
                idioma = IDIOMA_AQUAN;
            } else if ( tmp_msg == Idiomas_SHORTCUT_AURAN ) {
                idioma = IDIOMA_AURAN;
            } else if ( tmp_msg == Idiomas_SHORTCUT_IGNAN ) {
                idioma = IDIOMA_IGNAN;
            } else if ( tmp_msg == Idiomas_SHORTCUT_TERRAN ) {
                idioma = IDIOMA_TERRAN;
            } else if ( tmp_msg == Idiomas_SHORTCUT_GIGANTE ) {
                idioma = IDIOMA_GIGANTE;
            } else if ( tmp_msg == Idiomas_SHORTCUT_GNOLL ) {
                idioma = IDIOMA_GNOLL;
            } else if ( tmp_msg == Idiomas_SHORTCUT_SILVANO ) {
                idioma = IDIOMA_SILVANO;
            } else if ( tmp_msg == Idiomas_SHORTCUT_BAJOCOMUN ) {
                idioma = IDIOMA_BAJOCOMUN;
            }
        }
    }
    return idioma;
...
{
    string sEmote1 = "*fuma una pipa";
    string sEmote2 = "*inhala de una pipa*";
    string sEmote3 = "*exhala humo de una pipa*";
    float fHeight = 1.7;
    float fDistance = 0.1;
...
void ParseEmote(string sEmote, object oPC)
{
    ...
    {
        sRoll = GetStringRight(sEmote, GetStringLength(sEmote)-1);
        while (GetStringRight(sRoll, 1) == "*" && GetStringLength(sRoll) > 1)
        {
            sRoll = GetStringLeft(sRoll, GetStringLength(sRoll)-1);
        }
        sRoll = GetStringUpperCase(sRoll);
    ...
    //*emote* rolls
    if (sRoll == "FUERZA")
        SetLocalInt(oPC, "dmfi_univ_int", 41);
    else if (sRoll == "DESTREZA")
        SetLocalInt(oPC, "dmfi_univ_int", 42);
    else if (sRoll == "CONSTITUCION" ||
           sRoll == "CONSTITUCIÓN")
        SetLocalInt(oPC, "dmfi_univ_int", 43);
    else if (sRoll == "INTELIGENCIA")
        SetLocalInt(oPC, "dmfi_univ_int", 44);
    else if (sRoll == "SABIDURIA" ||
            sRoll == "SABIDURÍA")
        SetLocalInt(oPC, "dmfi_univ_int", 45);
    else if (sRoll == "CARISMA")
        SetLocalInt(oPC, "dmfi_univ_int", 46);
    else if (sRoll == "FORTALEZA" ||
             sRoll == "SALVACION DE FORTALEZA")
        SetLocalInt(oPC, "dmfi_univ_int", 47);
    else if (sRoll == "REFLEJOS" ||
            sRoll == "SALVACION DE REFLEJOS")
        SetLocalInt(oPC, "dmfi_univ_int", 48);
    else if (sRoll == "VOLUNTAD" ||
            sRoll == "SALVACION DE VOLUNTAD")
        SetLocalInt(oPC, "dmfi_univ_int", 49);
    else if (sRoll == "TRATO CON ANIMALES")
        SetLocalInt(oPC, "dmfi_univ_int", 89);
    else if (sRoll == "TASACION" ||
            sRoll == "TASACIÓN")
        SetLocalInt(oPC, "dmfi_univ_int", 88);
     else if (sRoll == "ENGAÑAR" ||
             sRoll == "ENGANIAR")
        SetLocalInt(oPC, "dmfi_univ_int", 61);
    else if (sRoll == "CONCENTRACION" ||
            sRoll == "CONCENTRACIÓN")
        SetLocalInt(oPC, "dmfi_univ_int", 56);
     else if (sRoll == "FABRICAR ARMADURA")
        SetLocalInt(oPC, "dmfi_univ_int", 68);
     else if (sRoll == "FABRICAR TRAMPA")
        SetLocalInt(oPC, "dmfi_univ_int", 69);
     else if (sRoll == "FABRICAR ARMA")
        SetLocalInt(oPC, "dmfi_univ_int", 67);
     else if (sRoll == "DESARMAR TRAMPA" ||
             sRoll == "INUTILIZAR MECANISMO")
        SetLocalInt(oPC, "dmfi_univ_int", 72);
    else if (sRoll == "SUJETAR")
        SetLocalInt(oPC, "dmfi_univ_int", 86);
    else if (sRoll == "SANAR" ||
             sRoll == "CURAR")
        SetLocalInt(oPC, "dmfi_univ_int", 85);
    else if (sRoll == "ESCONDERSE")
        SetLocalInt(oPC, "dmfi_univ_int", 64);
    else if (sRoll == "INTIMIDAR")
        SetLocalInt(oPC, "dmfi_univ_int", 74);
    else if (sRoll == "ESCUCHAR")
        SetLocalInt(oPC, "dmfi_univ_int", 65);
    else if (sRoll == "MOVERSE SIGILOSAMENTE")
        SetLocalInt(oPC, "dmfi_univ_int", 76);
    else if (sRoll == "ABRIR CERRADURAS")
        SetLocalInt(oPC, "dmfi_univ_int", 51);
    else if (sRoll == "ESGRIMA")
        SetLocalInt(oPC, "dmfi_univ_int", 66);
    else if (sRoll == "INTERPRETAR")
        SetLocalInt(oPC, "dmfi_univ_int", 73);
    else if (sRoll == "DIPLOMACIA")
        SetLocalInt(oPC, "dmfi_univ_int", 58);
    else if (sRoll == "JUEGO DE MANOS")
        SetLocalInt(oPC, "dmfi_univ_int", 75);
    else if (sRoll == "BUSCAR")
        SetLocalInt(oPC, "dmfi_univ_int", 54);
    else if (sRoll == "CONOCIMIENTO DE CONJUROS" ||
             sRoll == "SPELLCRAFT")
        SetLocalInt(oPC, "dmfi_univ_int", 57);
    else if (sRoll == "AVISTAR")
        SetLocalInt(oPC, "dmfi_univ_int", 53);
    else if (sRoll == "PROVOCAR")
        SetLocalInt(oPC, "dmfi_univ_int", 81);
    else if (sRoll == "PIRUETAS")
        SetLocalInt(oPC, "dmfi_univ_int", 79);
    else if (sRoll == "USAR DISPOSITIVO MAGICO" ||
            sRoll == "USAR DISPOSITIVO MÁGICO" ||
            sRoll == "USAR OBJETO MAGICO")
        SetLocalInt(oPC, "dmfi_univ_int", 92);
    else if (sRoll == "AVERIGUAR INTENCIONES")
        SetLocalInt(oPC, "dmfi_univ_int", 52);
    else if (sRoll == "CABALGAR")
        SetLocalInt(oPC, "dmfi_univ_int", 55);
    else if (sRoll == "DISFRAZARSE")
        SetLocalInt(oPC, "dmfi_univ_int", 59);
    else if (sRoll == "EQUILIBRIO")
        SetLocalInt(oPC, "dmfi_univ_int", 62);
    else if (sRoll == "ESCAPISMO")
        SetLocalInt(oPC, "dmfi_univ_int", 63);
    else if (sRoll == "NADAR")
        SetLocalInt(oPC, "dmfi_univ_int", 77);
    else if (sRoll == "OFICIO")
        SetLocalInt(oPC, "dmfi_univ_int", 78);
    else if (sRoll == "REUNIR INFORMACION" ||
             sRoll == "REUNIR INFORMACIÓN")
        SetLocalInt(oPC, "dmfi_univ_int", 82);
    else if (sRoll == "SALTAR")
        SetLocalInt(oPC, "dmfi_univ_int", 84);
    else if (sRoll == "SUPERVIVENCIA")
        SetLocalInt(oPC, "dmfi_univ_int", 87);
    else if (sRoll == "TREPAR")
        SetLocalInt(oPC, "dmfi_univ_int", 91);
    else if (sRoll == "USO DE CUERDAS" ||
             sRoll == "USAR CUERDAS")
        SetLocalInt(oPC, "dmfi_univ_int", 93);
    else if (sRoll == "NIVEL")
        SetLocalInt(oPC, "dmfi_univ_int", 94);
    else if (sRoll == "CONOCIMIENTO BARDICO" ||
             sRoll == "CONOCIMIENTO BÁRDICO" ||
             sRoll == "CONOCIMIENTO DE BARDO")
    ...
    //*emote*
    if (FindSubString(GetStringLowerCase(sEmote), "hace una reverencia") != -1 ||
        FindSubString(GetStringLowerCase(sEmote), "reverencia") != -1)
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_BOW, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "bebe") != -1 ||
             FindSubString(GetStringLowerCase(sEmote), "sorbe") != -1)
            AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_DRINK, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "bebe") != -1 &&
             FindSubString(GetStringLowerCase(sEmote), "se sienta") != -1)
             {
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "lee") != -1 &&
             FindSubString(GetStringLowerCase(sEmote), "se sienta") != -1)
             {
                AssignCommand(oPC, ActionPlayAnimation( ANIMATION_LOOPING_SIT_CROSS, 1.0, 99999.0f));
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "se sienta")!= -1)
            {
            AssignCommand(oPC, ActionPlayAnimation( ANIMATION_LOOPING_SIT_CROSS, 1.0, 99999.0f));
            }
    else if (FindSubString(GetStringLowerCase(sEmote), "saluda")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "wave") != -1)
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_GREETING, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "bosteza")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "se estira") != -1 ||
             FindSubString(GetStringLowerCase(sEmote), "aburrido") != -1)
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_PAUSE_BORED, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "se rasca")!= -1)
    {
        ...
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_PAUSE_SCRATCH_HEAD, 1.0));
             ...
             FindSubString(GetStringLowerCase(sEmote), " lee")!= -1||
             FindSubString(GetStringLowerCase(sEmote), "lee")!= -1)
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_READ, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "saludar")!= -1)
    {
        AssignCommand(oPC,ActionUnequipItem(oRightHand));
        AssignCommand(oPC,ActionUnequipItem(oLeftHand));
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_SALUTE, 1.0));
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "robar")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "swipe") != -1)
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_STEAL, 1.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "provocar")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "se mofa") != -1)
    {
        PlayVoiceChat(VOICE_CHAT_TAUNT, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_TAUNT, 1.0));
    }
    else if ((FindSubString(GetStringLowerCase(sEmote), "fuma") != -1)||
            (FindSubString(GetStringLowerCase(sEmote), "fumar") != -1))
    {
        SmokePipe(oPC);
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "festeja ")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "festejar*")!= -1)
    {
        PlayVoiceChat(VOICE_CHAT_CHEER, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_VICTORY1, 1.0));
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "hurra")!= -1)
    {
        PlayVoiceChat(VOICE_CHAT_CHEER, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_VICTORY2, 1.0));
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "celebrar")!= -1)
    {
        PlayVoiceChat(VOICE_CHAT_CHEER, oPC);
        AssignCommand(oPC, PlayAnimation(ANIMATION_FIREFORGET_VICTORY3, 1.0));
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "se rie")!= -1 && GetGender(oPC) == GENDER_FEMALE)
        AssignCommand(oPC, PlaySound("vs_fshaldrf_haha"));
    else if (FindSubString(GetStringLowerCase(sEmote), "Zero")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "tropieza")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_DEAD_FRONT, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "se agacha")!= -1 ||
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "asiente")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "de acuerdo")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_LISTEN, 1.0, 4.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "escudriña")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "revisa")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "busca")!= -1)
        AssignCommand(oPC,ActionPlayAnimation(ANIMATION_LOOPING_LOOK_FAR, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "*reza")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), " reza")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "*medita")!= -1)
    {
        AssignCommand(oPC,ActionUnequipItem(oRightHand));
        AssignCommand(oPC,ActionUnequipItem(oLeftHand));
        AssignCommand(oPC,ActionPlayAnimation(ANIMATION_LOOPING_MEDITATE, 1.0, 99999.0));
    }
    else if (FindSubString(GetStringLowerCase(sEmote), "borracho")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "mareado")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_PAUSE_DRUNK, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "cansado")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "fatigado")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "exhausto")!= -1)
    {
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "exige")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "amenaza")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_TALK_FORCEFUL, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "carcajada")!= -1 ||
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "ruega")!= -1 ||
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "adora")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_WORSHIP, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "ronca")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "*duerme")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), " duerme")!= -1||
             FindSubString(GetStringLowerCase(sEmote), "duerme")!= -1)
        ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_IMP_SLEEP), oPC);
    else if (FindSubString(GetStringLowerCase(sEmote), "*canta")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), " canta")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "tararea")!= -1)
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_BARD_SONG), oPC, 6.0f);
    else if (FindSubString(GetStringLowerCase(sEmote), "silba")!= -1)
        AssignCommand(oPC, PlaySound("as_pl_whistle2"));
    else if (FindSubString(GetStringLowerCase(sEmote), "habla")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "charla")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_TALK_NORMAL, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "mira a los lados")!= -1)
    {
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "esquiva")!= -1)
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "lanza un conjuro")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_CONJURE1, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "prepara un conjuro")!= -1 ||
            FindSubString(GetStringLowerCase(sEmote), "prepara un conjuro")!= -1)
        AssignCommand(oPC, ActionPlayAnimation(ANIMATION_LOOPING_CONJURE2, 1.0, 99999.0));
    else if (FindSubString(GetStringLowerCase(sEmote), "plop")!= -1 ||
             FindSubString(GetStringLowerCase(sEmote), "se cae")!= -1 &&
    ...
    else if (FindSubString(GetStringLowerCase(sEmote), "espasmo")!= -1)
...
string ProcessCant(string sLetter)
{
    ...
    if (sLetter == "a" || sLetter == "A") return "*cierra los ojos*";
    if (sLetter == "b" || sLetter == "B") return "*bufa*";
    if (sLetter == "c" || sLetter == "C") return "*tose*";
    if (sLetter == "d" || sLetter == "D") return "*arquea una ceja*";
    if (sLetter == "e" || sLetter == "E") return "*mira el piso*";
    if (sLetter == "f" || sLetter == "F") return "*frunce el ceño*";
    if (sLetter == "g" || sLetter == "G") return "*echa un vistazo*";
    if (sLetter == "h" || sLetter == "H") return "*parece pensativo*";
    if (sLetter == "i" || sLetter == "I") return "*parece aburrido*";
    if (sLetter == "j" || sLetter == "J") return "*se frota la barbilla*";
    if (sLetter == "k" || sLetter == "K") return "*se rasca una oreja*";
    if (sLetter == "l" || sLetter == "L") return "*mira alrededor*";
    if (sLetter == "m" || sLetter == "M") return "*mmm*";
    if (sLetter == "n" || sLetter == "N") return "*asiente*";
    if (sLetter == "o" || sLetter == "O") return "*sonrie abiertamente*";
    if (sLetter == "p" || sLetter == "P") return "*sonrie*";
    if (sLetter == "q" || sLetter == "Q") return "*le da un escalofrio*";
    if (sLetter == "r" || sLetter == "R") return "*gira los ojos*";
    if (sLetter == "s" || sLetter == "S") return "*se rasca la nariz*";
    if (sLetter == "t" || sLetter == "T") return "*gira un poco*";
    if (sLetter == "u" || sLetter == "U") return "*mira distraidamente*";
    if (sLetter == "v" || sLetter == "V") return "*se pasa la mano por el cabello*";
    if (sLetter == "w" || sLetter == "W") return "*extiende la mano*";
    if (sLetter == "x" || sLetter == "X") return "*se estira*";
    if (sLetter == "y" || sLetter == "Y") return "*bosteza*";
    if (sLetter == "z" || sLetter == "Z") return "*se encoje de hombros*";
...
int GetDefaultRacialLanguage(object oPC, int iRename)
{
    ...
    {
        case RACIAL_TYPE_DWARF: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Enano"); return 4; break;
        case RACIAL_TYPE_ELF:
        case RACIAL_TYPE_HALFELF: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Elfico"); return 1; break;
        case RACIAL_TYPE_GNOME: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Gnomico"); return 2; break;
        case RACIAL_TYPE_HALFLING: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Mediano"); return 3; break;
        case RACIAL_TYPE_HUMANOID_ORC:
        case RACIAL_TYPE_HALFORC: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Orco"); return 5; break;
        case RACIAL_TYPE_HUMANOID_GOBLINOID: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Goblin"); return 6; break;
        case RACIAL_TYPE_HUMANOID_REPTILIAN:
        case RACIAL_TYPE_DRAGON: if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Draconico"); return 7; break;
        default: if (GetLevelByClass(CLASS_TYPE_RANGER, oPC) || GetLevelByClass(CLASS_TYPE_DRUID, oPC))
                 ...
                 {
                    if (iRename) SetLocalString(oPC, "hls_MyLanguageName", "Jerga de Ladron");
                    return 9;
                 }
                 break;
    }
...
void ParseCommand(object oNPC, object oPC, string sCom)
{
    ...
    if (!GetIsObjectValid(oNPC))
        {
        SendMessageToPC(oPC, "No current voice command target - no voice targetting commands will function.");
        return;
        }

    if (GetStringLowerCase(GetStringLeft(sCom, 12)) ==".appearance ")
    {
        string sNew = GetStringRight(sCom, GetStringLength(sCom)-12);
        SendMessageToPC(oPC, "Parse command: " + sNew);
        int Appear = AppearType(sNew);

        if (Appear!=-1)
           {SetCreatureAppearanceType(GetLocalObject(oPC, "dmfi_univ_target"), Appear);}
        else
           {FloatingTextStringOnCreature("Invalid Appearance Type", oPC);}
    ...
    if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".loc")
        {
        SetLocalInt(oPC, "dmfi_dicebag", 2);
        SetCustomToken(20681, "Local");
        SetCampaignInt("dmfi", "dmfi_dicebag", 2, oPC);
        FloatingTextStringOnCreature("Broadcast Mode set to Local", oPC, FALSE); return;
        }
    if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".glo")
        {
        SetLocalInt(oPC, "dmfi_dicebag", 1);
        SetCustomToken(20681, "Global");
        SetCampaignInt("dmfi", "dmfi_dicebag", 1, oPC);
        FloatingTextStringOnCreature("Broadcast Mode set to Global", oPC, FALSE); return;
        }
    if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".pri")
        {
        SetLocalInt(oPC, "dmfi_dicebag", 0);
        SetCustomToken(20681, "Private");
        SetCampaignInt("dmfi", "dmfi_dicebag", 0, oPC);
        FloatingTextStringOnCreature("Broadcast Mode set to Private", oPC, FALSE); return;
        }
     if (GetStringLowerCase(GetStringLeft(sCom, 3)) == ".dm")
        {
        SetLocalInt(oPC, "dmfi_dicebag", 3);
        SetCustomToken(20681, "DM Only");
        SetCampaignInt("dmfi", "dmfi_dicebag", 3, oPC);
        FloatingTextStringOnCreature("Broadcast Mode set to DM Only", oPC, FALSE); return;
        }
    if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".aniy")
        {
        SetLocalInt(oPC, "dmfi_dice_no_animate", 0);
        FloatingTextStringOnCreature("Rolls will show animation", oPC, FALSE); return;
        }
    if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".anin")
        {
        SetLocalInt(oPC, "dmfi_dice_no_animate", 1);
        FloatingTextStringOnCreature("Rolls will NOT show animation", oPC, FALSE); return;
        }

    if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".stre")
       iOffset=  11;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".dext")
       iOffset = 12;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".cons")
       iOffset = 13;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".inte")
       iOffset = 14;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".wisd")
       iOffset = 15;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".char")
       iOffset = 16;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".fort")
       iOffset = 17;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".refl")
       iOffset = 18;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".anim")
       iOffset = 21;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".appr")
       iOffset = 22;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".bluf")
       iOffset =  23;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".conc")
       iOffset = 24;
    else if (GetStringLowerCase(GetStringLeft(sCom, 9)) == ".craft ar")
       iOffset =  25;
    else if (GetStringLowerCase(GetStringLeft(sCom, 9)) == ".craft tr")
       iOffset =  26;
    else if (GetStringLowerCase(GetStringLeft(sCom, 9)) == ".craft we")
       iOffset =  27;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".disa")
       iOffset =  28;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".disc")
       iOffset =  29;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".heal")
       iOffset =  31;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".hide")
       iOffset =  32;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".inti")
       iOffset =  33;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".list")
       iOffset =  34;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".lore")
       iOffset =  35;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".move")
       iOffset =  36;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".open")
       iOffset =   37;
     else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".parr")
       iOffset =  38;
     else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".perf")
       iOffset =  39;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".pers")
       iOffset =  41;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".pick")
       iOffset =  42;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".sear")
       iOffset =  43;
    else if (GetStringLowerCase(GetStringLeft(sCom, 6)) == ".set t")
       iOffset =  44;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".spel")
       iOffset =  45;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".spot")
       iOffset =  46;
     else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".taun")
       iOffset =   47;
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".tumb")
       iOffset =  48;
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".use")
       iOffset =   49;

    if (iOffset!=0)
    {
        if (FindSubString(GetStringLowerCase(sCom), "all") != -1)
            SetLocalInt(oPC, "dmfi_univ_int", iOffset+40);
            else
            SetLocalInt(oPC, "dmfi_univ_int", iOffset);

        SetLocalString(oPC, "dmfi_univ_conv", "dicebag");
        //SetLocalObject(oPC, "dmfi_univ_target", oNPC);
        //should still send last DMFI target
        if (!GetIsObjectValid(GetLocalObject(oPC, "dmfi_univ_target")))
            {
            SetLocalObject(oPC, "dmfi_univ_target", oNPC);
            SendMessageToPC(oPC, "No valid DMFI target:  Transferred to voice widget target");
            }
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".ani")
    {
        sCom = GetStringRight(sCom, GetStringLength(sCom) - 4);
        ...
        if (sCom != "" && GetStringLeft(sCom, 1) == " ")
        {
            ...
            AssignCommand(oNPC, ActionPlayAnimation(iCom, 1.0, 99999.0f));
            return;
        }
        return;
    }
    ...
    {
        if (FindSubString(GetStringLowerCase(sCom), "low") !=-1)
            {
            ...
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_RESISTANCE, oNPC, METAMAGIC_ANY, TRUE, 5, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_GHOSTLY_VISAGE, oNPC, METAMAGIC_ANY, TRUE, 5, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_CLARITY,  oNPC,METAMAGIC_ANY, TRUE, 5, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            FloatingTextStringOnCreature("Low Buff applied: " + GetName(oNPC), oPC);   return;
            }
        else if (FindSubString(GetStringLowerCase(sCom), "mid") !=-1)
            {
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_LESSER_SPELL_MANTLE, oNPC, METAMAGIC_ANY, TRUE, 10, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_STONESKIN, oNPC, METAMAGIC_ANY, TRUE, 10, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_ELEMENTAL_SHIELD,  oNPC,METAMAGIC_ANY, TRUE, 10, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            FloatingTextStringOnCreature("Mid Buff applied: " + GetName(oNPC), oPC);  return;
            }
         else if (FindSubString(GetStringLowerCase(sCom), "high") !=-1)
            {
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_SPELL_MANTLE, oNPC, METAMAGIC_ANY, TRUE, 15, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_STONESKIN, oNPC, METAMAGIC_ANY, TRUE,15, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_SHADOW_SHIELD,  oNPC,METAMAGIC_ANY, TRUE, 15, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            FloatingTextStringOnCreature("High Buff applied: " + GetName(oNPC), oPC);  return;
            }
        else if (FindSubString(GetStringLowerCase(sCom), "epic") !=-1)
            {
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_GREATER_SPELL_MANTLE, oNPC, METAMAGIC_ANY, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_SPELL_RESISTANCE, oNPC, METAMAGIC_ANY, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_SHADOW_SHIELD,  oNPC,METAMAGIC_ANY, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            AssignCommand(oNPC, ActionCastSpellAtObject(SPELL_CLARITY,  oNPC,METAMAGIC_ANY, TRUE, 20, PROJECTILE_PATH_TYPE_DEFAULT, TRUE));
            FloatingTextStringOnCreature("Epic Buff applied: " + GetName(oNPC), oPC);  return;
            }
        else if (FindSubString(GetStringLowerCase(sCom), "barkskin") != -1)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectACIncrease(3, AC_NATURAL_BONUS), oNPC, 3600.0f);
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_PROT_BARKSKIN), oNPC, 3600.0f);  return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "elements") != -1)
        {
            ...
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_PROTECTION_ELEMENTS), oNPC, 3600.0f);  return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "haste") != -1)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectHaste(), oNPC, 3600.0f);  return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "invis") != -1)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectInvisibility(INVISIBILITY_TYPE_NORMAL), oNPC, 3600.0f);   return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "unplot") != -1)
        {
            SetPlotFlag(oNPC, FALSE);
            FloatingTextStringOnCreature("The target is set to non-Plot.", oPC, FALSE); return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "plot") != -1)
        {
            SetPlotFlag(oNPC, TRUE);
            FloatingTextStringOnCreature("The target is set to Plot.", oPC, FALSE);  return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "stoneskin") != -1)
        {
            ...
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectVisualEffect(VFX_DUR_PROT_GREATER_STONESKIN), oNPC, 3600.0f); return;
        }
        else if (FindSubString(GetStringLowerCase(sCom), "trues") != -1)
        {
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectTrueSeeing(), oNPC, 3600.0f); return;
        }
    }
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 5)) == ".dism")
    {
        DestroyObject(oNPC);
        FloatingTextStringOnCreature(GetName(oNPC) + " dismissed", oPC, FALSE); return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".inv")
    {
        OpenInventory(oNPC, oPC);
        FloatingTextStringOnCreature(GetName(oNPC) + " dismissed", oPC, FALSE); return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".dmt")
    {
    ...
    {
        SetCampaignInt("dmfi", "dmfi_DMSpy", abs(GetCampaignInt("dmfi", "dmfi_DMSpy", oPC) -1), oPC); return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".fac")
    {
        if (FindSubString(GetStringLowerCase(sCom), "hostile") != -1)
        {
            ...
            if (GetIsImmune(oNPC, IMMUNITY_TYPE_BLINDNESS))
            {
            SendMessageToPC(oPC, "Targeted creature is blind immune - no attack will occur until new perception event is fired");
            return;
            }
            else
            {
            ...
            SendMessageToPC(oPC, "Faction Adjusted - will take effect in 6 seconds");
            return;
           }
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".get")
    {
        ...
        while (sCom != "")
        {
            ...
            {
                object oJump = GetLocalObject(GetModule(), "hls_NPCControl" + GetStringLeft(sCom, 1));
                if (GetIsObjectValid(oJump))
                {
                    AssignCommand(oJump, ClearAllActions());
                    AssignCommand(oJump, ActionJumpToLocation(GetLocation(oPC)));
                    return;
                }
                else
                    FloatingTextStringOnCreature("Your Control Character is not valid. Perhaps you are using a reserved character.", oPC, FALSE);
        ...
        while (sCom != "")
        {
            ...
            else
            {
                ...
                if (GetIsObjectValid(oJump))
                {
                    ...
                    AssignCommand(oPC, ActionJumpToLocation(GetLocation(oJump)));
                }
                else
                    FloatingTextStringOnCreature("Your Control Character is not valid. Perhaps you are using a reserved character.", oPC, FALSE);
                return;
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".ite")
    {
        ...
        {
            sCom = GetStringRight(sCom, GetStringLength(sCom) - 1);
            object oCreate = CreateItemOnObject(sCom, oNPC, 1);
            if (GetIsObjectValid(oCreate))
                FloatingTextStringOnCreature("Item " + GetName(oCreate) + " created.", oPC, FALSE);
            return;
        }
        return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".lan") //sets the language of the target
    {
        ...
        {
            SetLocalInt(oNPC, "hls_MyLanguage", 12);
            SetLocalString(oNPC, "hls_MyLanguageName", "Drow");
            FloatingTextStringOnCreature("Language set to Drow", oPC, FALSE);
        }
    return;
    }
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".npc")
    {
        ...
        {
            sCom = GetStringRight(sCom, GetStringLength(sCom) - 1);
            object oCreate = CreateObject(OBJECT_TYPE_CREATURE, sCom, GetLocation(oNPC));
            if (GetIsObjectValid(oCreate))
                FloatingTextStringOnCreature("NPC " + GetName(oCreate) + " created.", oPC, FALSE);
            return;
        }
        return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".pla")
    {
        ...
        {
            sCom = GetStringRight(sCom, GetStringLength(sCom) - 1);
            object oCreate = CreateObject(OBJECT_TYPE_PLACEABLE, sCom, GetLocation(oNPC));
            if (GetIsObjectValid(oCreate))
                FloatingTextStringOnCreature("Placeable " + GetName(oCreate) + " created.", oPC, FALSE);
            return;
        }
        return;
    }
    ...
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".say")
    {
        ...
        int iCom = StringToInt(sCom);
        if (GetCampaignString("dmfi", "hls206" + IntToString(iCom)) != "")
        {
            AssignCommand(oNPC, SpeakString(GetCampaignString("dmfi", "hls206" + IntToString(iCom))));
        }
        return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".tar")
    {
        object oGet = GetFirstObjectInArea(GetArea(oPC));
        if (GetStringLeft(sCom, 5) == ".tar ") sCom = GetStringRight(sCom, GetStringLength(sCom) - 5);
        else if (GetStringLeft(sCom, 8) == ".target ") sCom = GetStringRight(sCom, GetStringLength(sCom) - 8);
        while (GetIsObjectValid(oGet))
        {
            if (FindSubString(GetName(oGet), sCom) != -1)
            {
                SetLocalObject(oPC, "dmfi_VoiceTarget", oGet);
                FloatingTextStringOnCreature("You have targeted " + GetName(oGet) + " with the Voice Widget", oPC, FALSE);
                return;
            }
            oGet = GetNextObjectInArea(GetArea(oPC));
        }
        FloatingTextStringOnCreature("Target not found.", oPC, FALSE);
        return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".unf")
    {
        FloatingTextStringOnCreature(GetName(oNPC) + " unfrozen", oPC, FALSE);
        SetCommandable(TRUE, oNPC); return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".unm")
    {
        FloatingTextStringOnCreature(GetName(oNPC) + " un-muted", oPC, FALSE);
        DeleteLocalInt(oNPC, "dmfi_Mute"); return;
    }
    else if (GetStringLowerCase(GetStringLeft(sCom, 4)) == ".vfx")
    {
        sCom = GetStringRight(sCom, GetStringLength(sCom) - 4);
        //Parses the characters until there is a space.
        while (GetStringLeft(sCom, 1) != " " && sCom != "")
        ...
        if (sCom != "" && GetStringLeft(sCom, 1) == " ")
        {
            sCom = GetStringRight(sCom, GetStringLength(sCom) - 1);
            int iCom = StringToInt(sCom);
            if (GetTag(oNPC) == "dmfi_voice")
                ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(iCom), GetLocation(oNPC), 10.0f);
            else
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(iCom), oNPC, 10.0f);
            return;
        }
        return;
    }
...
struct ChatMessage Idiomas_traducir( struct ChatMessage message );
struct ChatMessage Idiomas_traducir( struct ChatMessage message )
{
    struct ChatMessage translatedMsg;
    translatedMsg.speaker = message.speaker;
    translatedMsg.volume = message.volume;
    translatedMsg.content = message.content;
    //Gets the current language that the character is speaking
    int iTranslate = Idiomas_getLanguageFromShortcut(translatedMsg.content);
    iTranslate = ( Idiomas_pjSabeIdioma(translatedMsg.speaker, iTranslate) ) ? iTranslate : IDIOMA_COMUN;
    if ( iTranslate != IDIOMA_COMUN ) {
        translatedMsg.content = GetStringRight( translatedMsg.content, GetStringLength(translatedMsg.content)-1 );
        translatedMsg.content = GetStringRight( translatedMsg.content, GetStringLength(translatedMsg.content)-FindSubString(translatedMsg.content, "*") );
    } else if ( GetLocalInt(translatedMsg.speaker, "hls_MyLanguage") && GetStringLeft(translatedMsg.content, 1) == "[" ) {
        iTranslate = GetLocalInt(translatedMsg.speaker, "hls_MyLanguage");
    } else {
    return message; // si es idioma comun, termina aca, podria emprolijarse, pero ya estaba asi. nota by drago
    }
    //Defines language name
    string sLanguageName = GetLocalString(translatedMsg.speaker, "hls_MyLanguageName");

    if ( GetStringLeft(translatedMsg.content, 1) == " " || GetStringLeft(translatedMsg.content, 1) == "[" ) {
        translatedMsg.content = GetStringRight(translatedMsg.content, GetStringLength(translatedMsg.content)-1);
    }
    //Thieves' Cant character limit of 25
    if (iTranslate == 9 && GetStringLength(translatedMsg.content) > 25)
        translatedMsg.content = GetStringLeft(translatedMsg.content, 25);
    string sSpeak = TranslateCommonToLanguage(iTranslate, translatedMsg.content);
    if (GetStringRight(translatedMsg.content, 1) == "]")
        translatedMsg.content = GetStringLeft(translatedMsg.content, GetStringLength(translatedMsg.content)-1);

    Idiomas_sendMessageToAllDMs( GetName(translatedMsg.speaker) + " dice en " + sLanguageName + ": " + translatedMsg.content );

    object oEavesdrop = GetFirstObjectInShape(SHAPE_SPHERE, 20.0f, GetLocation(translatedMsg.speaker), FALSE, OBJECT_TYPE_CREATURE);
    while (GetIsObjectValid(oEavesdrop))
    {
        if (GetIsPC(oEavesdrop) && !GetIsDM(oEavesdrop))
        {
            ...
            if (GetIsObjectValid(GetItemPossessedBy(oEavesdrop, "hlslang_" + IntToString(iTranslate))) ||
                ...
                GetHasFeat(FEAT_MONK_TONGUE_SUN_MOON, oEavesdrop)) {
                    SendMessageToPC(oEavesdrop, GetName(translatedMsg.speaker) + " dice en " + sLanguageName + ": " + translatedMsg.content);
            } else if (d20() + GetSkillRank(SKILL_LORE, oEavesdrop) > 20 && iTranslate != 9) {
                    SendMessageToPC(oEavesdrop, GetName(translatedMsg.speaker) + " esta hablando en " + sLanguageName);
            }
        }
        oEavesdrop = GetNextObjectInShape(SHAPE_SPHERE, 20.0f, GetLocation(translatedMsg.speaker), FALSE, OBJECT_TYPE_CREATURE);
    }
    //PrintString("<Conv>"+GetName(GetArea(translatedMsg.speaker))+ " " + GetName(translatedMsg.speaker) + " says in " + sLanguageName + ": " + translatedMsg.content + " </Conv>");
    return translatedMsg;
}


// Draco reemplazo esta funcion por "struct ChatMessage Idiomas_traducir( struct ChatMessage message )", pero como se olvido de actualizar las llamadas, Inquisidor la trajo desde la version 551.
// Draco, quitala cuando termines de cambiar las llamadas a esta funcion hacia 'Idiomas_traducir(..)'
...
void TranslateToLanguage(string sSaid, object oShouter)
{
    ...
    object oEavesdrop = GetFirstObjectInShape(SHAPE_SPHERE, 20.0f, GetLocation(oShouter), FALSE, OBJECT_TYPE_CREATURE);
    ...
    while (GetIsObjectValid(oEavesdrop))
    {
                ...
                if (GetIsObjectValid(GetItemPossessedBy(oEavesdrop, "hlslang_" + IntToString(iTranslate))) ||
                    ...
                    iTranslate == GetDefaultClassLanguage(oEavesdrop) ||
                    GetHasFeat(FEAT_MONK_TONGUE_SUN_MOON, oEavesdrop))
                        SendMessageToPC(oEavesdrop, GetName(oShouter) + " dice en " + sLanguageName + ": " + sSaid);
                else
                {
                    if (d20() + GetSkillRank(SKILL_LORE, oEavesdrop) > 20 && iTranslate != 9)
                        SendMessageToPC(oEavesdrop, GetName(oShouter) + " esta hablando en " + sLanguageName);
                }
                }
