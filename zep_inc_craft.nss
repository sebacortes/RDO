...
//:: Upated/Modified by CFX: 23-Jan-2005 - added 2 new routines to replace
//::                    redundant Set/Remove FLAG calls and stop ALL exploits!
//:: Updated/Modified by Eligio: 24-Jan-2005 - upped max weapon colors to 4
//:://////////////////////////////////////////////
...
//addition by CFX to set items to plot, stolen and cursed to stop exploiters
void ZEP_SetItemVariables (object oItem);

//addtion by CFX to remove plot, stolen and cursed from crafting items
void ZEP_RemoveItemVariables (object oItem);
...
//fleshed out anti-exploit routine -CFX
void ZEP_SetItemVariables(object oItem) {
    //sets the plot, stolen and cursed flags on the crafting item
...
//fleshed out routine to remove the temporary crafting flags set on crafted items -CFX
void ZEP_RemoveItemVariables(object oItem) {
    //removes the plot,cursed, and stolen flags set on the crafted item
...
void ZEP_StartCraft(object oPC, object oItem) {
    ...
    // Esto esta copiado aqui para que la balanza de intercambio
    // complete las transicion de items que estan en proceso.
    CIB_completarPosibleTransicionPropietario( oItem, oPC );
    ...
    object oBackup = CopyItem(oItem, oPC, TRUE);
    ...
    // Mark temporary item
    ZEP_SetItemVariables(oItem); // -cfx change
...
void ZEP_StopCraft(object oPC, int nExecute) {
    ...
    // Postclean settings
    DeleteLocalInt(oPC, "ZEP_CR_STARTED"); // -cfx change
    ...
    if (IPS_Item_getIsAdept(oItem))
        IPS_Item_enableProperties(oItem, oPC);
...
void ZEP_RemakeItem(object oPC, int nMode) {
    ...
    if (GetBaseItemType(oItem)==BASE_ITEM_ARMOR) {
        ...
        } while ( ( (nPart == ITEM_APPR_ARMOR_MODEL_BELT && nCurrApp==155) ||
                    ((nPart == ITEM_APPR_ARMOR_MODEL_LFOOT || nPart == ITEM_APPR_ARMOR_MODEL_RFOOT) && (nCurrApp >= 110 && nCurrApp <= 118)) ||
                    ((nPart == ITEM_APPR_ARMOR_MODEL_LSHIN || nPart == ITEM_APPR_ARMOR_MODEL_RSHIN) && (nCurrApp >= 110 && nCurrApp <= 117)) ||
                    ((nPart == ITEM_APPR_ARMOR_MODEL_LTHIGH || nPart == ITEM_APPR_ARMOR_MODEL_RTHIGH) && nCurrApp == 117) ||
                    (nPart == ITEM_APPR_ARMOR_MODEL_PELVIS && nCurrApp == 157) ||
                    (nPart == ITEM_APPR_ARMOR_MODEL_ROBE && (nCurrApp == 187 || nCurrApp == 186) && GetGender(oPC) == GENDER_MALE)
                  ) && (!GetIsDM(oPC) && !GetIsDMPossessed(oPC))
                 );
        //SendMessageToPC(oPC, "nCurrApp: "+sID);
        ...
        do {
            do {
                ...
                //SendMessageToPC(oPC, "nCurrApp: "+IntToString(nCurrApp));
            } while ( (nCurrApp >= 34 && nCurrApp <= 46) && (!GetIsDM(oPC) && !GetIsDMPossessed(oPC)) );
        ...
        do {
            do {
                ...
                //SendMessageToPC(oPC, "nCurrApp: "+IntToString(nCurrApp));
                        ...
                        (   (tipoBase == BASE_ITEM_SMALLSHIELD && (
                                                                    (nCurrApp >= 35 && nCurrApp <= 39) ||
                                                                    (nCurrApp >= 44 && nCurrApp <= 49) ||
                                                                    (nCurrApp >= 61 && nCurrApp <= 64) ||
                                                                    (nCurrApp >= 101 && nCurrApp <= 104)
                                                                  )
                            ) ||
                            (tipoBase == BASE_ITEM_LARGESHIELD && ( nCurrApp >= 44 && nCurrApp <= 49 )) ||
                            (tipoBase == BASE_ITEM_TOWERSHIELD && ((nCurrApp >= 14 && nCurrApp <= 19) || nCurrApp == 61))
                        ) && (!GetIsDM(oPC) && !GetIsDMPossessed(oPC))
                     );
        ...
        do {
            do {
                ...
                //SendMessageToPC(oPC, "nCurrApp: "+IntToString(nCurrApp));
                        ...
                        (
                            (tipoBase == BASE_ITEM_BASTARDSWORD     && nCurrApp == 20) ||
                            (tipoBase == BASE_ITEM_DAGGER           && nCurrApp == 19) ||
                            (tipoBase == BASE_ITEM_GREATSWORD       && nCurrApp == 16) ||
                            (tipoBase == BASE_ITEM_KATANA           && nCurrApp == 23) ||
                            (tipoBase == BASE_ITEM_LONGSWORD        && nCurrApp == 21) ||
                            (tipoBase == BASE_ITEM_RAPIER           && nCurrApp == 14) ||
                            (tipoBase == BASE_ITEM_SCIMITAR         && nCurrApp == 23) ||
                            (tipoBase == BASE_ITEM_SHORTSWORD       && nCurrApp == 20) ||
                            (tipoBase == BASE_ITEM_TWOBLADEDSWORD   && nCurrApp == 25)
                        ) && (!GetIsDM(oPC) && !GetIsDMPossessed(oPC))
                        && nPart == ITEM_APPR_WEAPON_MODEL_TOP
                     );
    ...
    if (GetIsObjectValid(oNew)) {
        ...
        // Agregado para evitar problemas con la identificacion del item por el IPS
        if (IPS_Item_getIsAdept(oNew))
            IPS_Item_disableProperties(oNew, oPC);
        ...
        ZEP_SetItemVariables(oNew); // -cfx change
        AssignCommand(oPC, ClearAllActions(TRUE));
...
void ZEP_RecolorItem(object oPC, int nMode) {
    ...
    if (GetBaseItemType(oItem)==BASE_ITEM_ARMOR || GetBaseItemType(oItem)==BASE_ITEM_HELMET) {
        ...
        int nMin = 0;
        int nMax = 175;         //64 colors in NWN Palette-Loki
...
//
        ...
        int nMin = 1;
        int nMax = 4;   //Eligio: v1.51 Jan 18 - now we have all 4 colors (fix identified by caesarbear)
