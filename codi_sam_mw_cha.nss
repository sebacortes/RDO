...
void main()
{
    ...
    if (TRUE)
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            ...
            {
                //SpeakString("Ability Strength: " + IntToString(ABILITY_CHARISMA));
                ...
                if(GetItemPropertySubType(ip) == ABILITY_CHARISMA)
                {
                    iValue = GetItemPropertyCostTableValue(ip);
                    RemoveItemProperty(oWeapon, ip);
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(ABILITY_CHARISMA, iValue + 1), oWeapon);
                    iBreak = TRUE;
                }
            }
        ...
        if(!iBreak)
        {
            ip = ItemPropertyAbilityBonus(ABILITY_CHARISMA, 1);
