...
************************* [Targeting] *****************************************/
    ...
    //            mage (attack higher!)
    if(GetBaseAttackBonus(OBJECT_SELF) > ((GetHitDice(OBJECT_SELF)/i2) + i1))
    {
...
************************* [Combat - Fighters] *********************************/
    ...
    //SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_DISPEL_WITH_MELEE, AI_OTHER_COMBAT_MASTER);
        // this mean effect dispel with melee attack or immunite damage shield.
    ...
    else if(!GetHasFeat(FEAT_WEAPON_FINESSE) && GetHasFeat(FEAT_POINT_BLANK_SHOT) &&
    (GetAbilityScore(OBJECT_SELF,ABILITY_DEXTERITY)>GetAbilityScore(OBJECT_SELF,ABILITY_STRENGTH) || GetHasFeat(FEAT_ZEN_ARCHERY)))
    {
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ATTACKING, AI_COMBAT_MASTER);
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ALWAYS_MOVE_BACK, AI_COMBAT_MASTER);
    }
    else if(!GetHasFeat(FEAT_WEAPON_FINESSE) &&
    (GetAbilityScore(OBJECT_SELF,ABILITY_DEXTERITY)>GetAbilityScore(OBJECT_SELF,ABILITY_STRENGTH) || GetHasFeat(FEAT_ZEN_ARCHERY)))
    {
        SetSpawnInCondition(AI_FLAG_COMBAT_ARCHER_ATTACKING, AI_COMBAT_MASTER);
    }
...
************************* [Combat - Spell Casters] ****************************/
    ...
    SetSpawnInCondition(AI_FLAG_COMBAT_COUNTER_SPELL_ONLY_IN_GROUP, AI_COMBAT_MASTER);
        ...
        // modified. when ally is more then enemy.
    ...
    //SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_ONLY_SELF_BUFFING_SPELLS, AI_OTHER_COMBAT_MASTER);
        // Buffing self only.
...
************************* [Combat - Dragons] **********************************/
    ...
    SetAIInteger(AI_DRAGON_FREQUENCY_OF_BUFFET, 5);
        // Min. Amount of Rounds between each buffet. See readme for counter defaults. Def: 3
    SetAIInteger(AI_DRAGON_FREQUENCY_OF_BREATH, 2);
...
//    SetSpawnInCondition(NW_FLAG_STEALTH, NW_GENERIC_MASTER);
    if(GetStandardFactionReputation(STANDARD_FACTION_HOSTILE)>=i90 && GetSkillRank(SKILL_HIDE)>GetHitDice(OBJECT_SELF)/i2)
    {
        SetSpawnInCondition(NW_FLAG_STEALTH, NW_GENERIC_MASTER);
...
************************* [User] **********************************************/
    ...
    AI_AttemptFeatSummonFamiliar();

//    SetLocalInt(OBJECT_SELF,RULE_VERSION,i3);
//    SetLocalInt(OBJECT_SELF,AI_IM_HENCHMAN,TRUE);
/************************ [User] **********************************************/
