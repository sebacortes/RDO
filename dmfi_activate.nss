...
//initialize the listening commands reminder
    if (GetLocalInt(GetModule(), "dmfi_voice_initial")!=1 && (GetLocalInt(oUser, "dmfi_reminded")!=1)
        &&(GetIsDM(oUser) || GetPCPublicCDKey(oUser)=="MLQCEL4J"))
            {
...
//remainder of settings are user based
    ...
    if ((GetLocalInt(oUser, "dmfi_initialized")!=1) && (GetIsDM(oUser) || GetPCPublicCDKey(oUser)=="MLQCEL4J"))
    {
...
//********************************END INITIALIZATION***************************
    ...
    if (GetStringLeft(sItemTag,8) == "hlslang_")
    {
            ...
            if (GetIsObjectValid(GetLocalObject(oUser, "dmfi_MyVoice")))
            {
                ...
                FloatingTextStringOnCreature("Has destruido tu voz anterior", oUser, FALSE);
            }
            ...
            DelayCommand(1.0f, FloatingTextStringOnCreature("Estas hablando " + GetName(oItem) + ". Escribe /dm [(lo que quieras decir entre corchetes)]", oUser, FALSE));
    ...
    if (GetStringLeft(sItemTag, 5) == "dmfi_")
    {
        ...
        if (GetIsDM(oUser) || GetIsDMPossessed(oUser) || GetPCPublicCDKey(oUser)=="MLQCEL4J")
            iPass = TRUE;
