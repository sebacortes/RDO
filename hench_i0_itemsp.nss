...
void InitializeItemSpells(int bPolymorphed, int bNotBuffOnly = TRUE)
{
    ...
    if (bSpellCastingProblem || bDisablingEffect)
    {
        if (bNotBuffOnly)
        {
...
{
    if (!onlyUseTalents && CU_GetHasSpell(spellID))
    {
    ...
    if (HasItemTalentSpell(spellID))
    {
        ...
        bFoundItemSpell = !CU_GetHasSpell(spellID);
        return TRUE;
    }
...
void ActionCastFixedSpellOnObject(int nSpell, object oTarget)
{
    ...
    // this always works normally using my system
    ...
    {
        CU_ActionCastSpellAtObject(nSpell, oTarget);
    }
}
...
void CastFixedSpellOnObject(int nSpell, object oTarget, int nSpellLevel)
{
    ...
    int bHasSpell = CU_GetHasSpell(nSpell);
    ...
    {
        CU_ActionUseTalentOnObject(GetPotionTalentSpell(nSpell), OBJECT_SELF);
        return;
    }
    ...
    if (itemPosition)
    {
        ...
        CU_ActionUseTalentOnObject(GetItemTalentSpell(nSpell), oTarget);
        return;
    }
    ActionCastFixedSpellOnObject(nSpell, oTarget);
}
...
void CastFixedSpellAtLocation(int nSpell, location loc, int nSpellLevel)
{
    ...
    if (itemPosition)
    {
        ...
        CU_ActionUseTalentAtLocation(GetItemTalentSpell(nSpell), loc);
        return;
    }
    ActionCastFixedSpellAtLocation(nSpell, loc);
}
