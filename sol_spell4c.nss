...
// End of Spell Cast Hook
    ...
    int nDC = 14+GetAbilityModifier(ABILITY_WISDOM,OBJECT_SELF);
    int nCasterLevel = Sol;
    int nDuration = Sol;
    effect eSummon;
    effect eGate;
    ...
    if (GetIsObjectValid(oTarget))
    {
        ...
        if(nRacial == RACIAL_TYPE_OUTSIDER)
        {
            if(!GetIsReactionTypeFriendly(oTarget))
            {
                ...
                //Make a Will save
                if(!PRCMySavingThrow(SAVING_THROW_WILL, oTarget, ((nDC+ GetChangesToSaveDC(oTarget,OBJECT_SELF))+2)))
                {
                    //Apply the linked effect
                    SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(nDuration/2),TRUE,-1,Sol);
                }
            }
        }
    }
