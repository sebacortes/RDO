...
void main()
{
    ...
    if (TRUE)
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            ...
            {
                //SpeakString("Ability Strength: " + IntToString(ABILITY_WISDOM));
                ...
                if(GetItemPropertySubType(ip) == ABILITY_WISDOM)
                {
                    ...
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(ABILITY_WISDOM, iValue + 1), oWeapon);
                    iBreak = TRUE;
                }
            }
        ...
        if(!iBreak)
        {
            ip = ItemPropertyAbilityBonus(ABILITY_WISDOM, 1);
