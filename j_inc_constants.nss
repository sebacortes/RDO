...
// d&d rule version(modified spells)
const string RULE_VERSION                           = "RULE_VERSION";

// for henchman AI
const string AI_IM_HENCHMAN                         = "AI_IM_HENCHMAN";
...
// Follow ally
const int AI_SPECIAL_ACTIONS_FOLLOW_ALLY            = 4;                        // CONSTANT
...
// timer for SetNumberOfSpellAndAbility
const string AI_TIMER_SET_NUMBER_S_A                = "AI_TIMER_SET_NUMBER_S_A";
...
// AOE spell checked
const string AI_CHECKED_AOE_SPELL                   = "AI_CHECKED_AOE_SPELL";
...
// for multiple calls of setweapons
const string AI_LAST_SETWEAPONS_TIME                = "AI_LAST_SETWEAPONS_TIME";
...
// need rest check
const string AI_NEEDREST                            = "AI_NEEDREST";

// fast buff check
const string AI_NEEDFASTBUFF                        = "AI_NEEDFASTBUFF";

// last flee position
const string AI_LAST_X                              = "AI_LAST_X";
const string AI_LAST_Y                              = "AI_LAST_TY";
const string AI_LAST_Z                              = "AI_LAST_Z";
const string AI_BLOCKED                             = "AI_BLOCKED";
const string AI_POSSIBLYBLOCKED                     = "AI_POSSIBLYBLOCKED";
const string AI_LAST_CALLED_TIME                    = "AI_LAST_CALLED_TIME";
const string AI_LAST_CASTED_AOE_SPELL               = "AI_LAST_CASTED_AOE_SPELL";
const string AI_LAST_CASTED_AOE_SPELL_NUM           = "AI_LAST_CASTED_AOE_SPELL_NUM";
const string AI_AOE_LOCATION_TARGETING_FAILED       = "AI_AOE_LOCATION_TARGETING_FAILED";
const string AI_SPELL_HIGH_LOW_TOGGLE               = "AI_SPELL_HIGH_LOW_TOGGLE";
...
const int AI_SPELLABILITY_WYRMWHITEBREATH           = 663;
const int AI_SPELLABILITY_WYRMBLACKBREATH           = 664;
const int AI_SPELLABILITY_WYRMREDBREATH             = 665;
const int AI_SPELLABILITY_WYRMGREENBREATH           = 666;
const int AI_SPELLABILITY_WYRMBLUEBREATH            = 667;
...
const int AI_SPELLABILITY_SHIFTER_BLUEBREATH        = 796;
const int AI_SPELLABILITY_SHIFTER_REDBREATH         = 797;
const int AI_SPELLABILITY_SHIFTER_GREENBREATH       = 798;
...
// 9 - modified - need to cast dispel to melee target!
const string DISPEL_THIS_TARGET                     = "DISPEL_THIS_TARGET";     // CONSTANT
...
// Set to whatever we follow.
const string AI_TO_FOLLOW                           = "AI_TO_FOLLOW";           // S.OBJECT
...
const string AI_WEAPON_GLOVES                       = "AI_WEAPON_GLOVES";
...
const string AI_WEAPON_UNARMED                      = "AI_WEAPON_UNARMED";
// End post-fixs as it were
const string WEAP_SIZE                              = "AI_WEAP_SIZE";           // CONSTANT
const string WEAP_DAMAGE                            = "AI_WEAP_DAMAGE";         // CONSTANT
// equiped weapon value
const string AI_EQUIPED_VALUE_RIGHT_HAND            = "AI_EQUIPED_VALUE_RIGHT_HAND";
const string AI_EQUIPED_VALUE_LEFT_HAND             = "AI_EQUIPED_VALUE_LEFT_HAND";
const string AI_EQUIPED_VALUE_GLOVES                = "AI_EQUIPED_VALUE_GLOVES";
...
const string AI_SAVED_DISPEL_TARGET = "AI_SAVED_DISPEL_TARGET";
//const string AI_LOADED_DISPEL_TARGET= "AI_LOADED_DISPEL_TARGET";
...
// count of no-spell round
const string AI_NOMAGICCOUNT = "AI_NOMAGICCOUNT";

// dispel interval second
const string AI_DISPEL_CASTED_TIMER = "AI_DISPEL_CASTED_TIMER";

// counter spell attempt
const string AI_COUNTERSPELL_USER = "AI_COUNTERSPELL_USER";

// last casted spell
const string AI_LAST_CASTED_SPELL_ID = "AI_LAST_CASTED_SPELL_ID";

// familiar/animal companion type
const string AI_FAMILIAR_TYPE = "AI_FAMILIAR_TYPE";
const string AI_COMPANION_TYPE = "AI_COMPANION_TYPE";

// who did preengaged resurrection?
const string AI_SET_RESURRECTION_USER = "AI_SET_RESURRECTION_USER";
...
// (Dragon) flying on
const int AI_FLAG_COMBAT_FLYING                         = 0x00000008;
...
// Buffing self only.
const int AI_FLAG_OTHER_COMBAT_ONLY_SELF_BUFFING_SPELLS     = 0x08000000;
// can dispel with melee attack - paladin.. etc
const int AI_FLAG_OTHER_COMBAT_DISPEL_WITH_MELEE            = 0x10000000;
...
// number of spells has
const string AI_NUMBER_OF_SPELLS                                    = "AI_NUMBER_OF_SPELLS";
// number of spell-ability has
const string AI_NUMBER_OF_ABILITIES                                 = "AI_NUMBER_OF_ABILITIES";
...
// etc.etc.. get area width and height
int GetAreaWidth(object oArea);
int GetAreaHeight(object oArea);

// set number of spells and abilities
void SetNumberOfSpellAndAbility();
...
int AI_AttemptFeatSummonFamiliar(int nEffect=FALSE);
...
void Search(object oTarget = OBJECT_INVALID)
{
    if(GetLocalInt(OBJECT_SELF,AI_IM_HENCHMAN))return;
    ...
    {
        // Stop now
        // clearallactions was called in detercombatround function
//        ClearAllActions();
        // Rest after combat?
/*
...
int GetIsPerformingSpecialAction()
{
    ...
    switch(iAction)
    {
        ...
        break;
        case AI_SPECIAL_ACTIONS_FOLLOW_ALLY:
        {
            object oFollow=GetLocalObject(OBJECT_SELF,AI_TO_FOLLOW);
            // if there is enemy that is seen or heard
            if(GetIsObjectValid(GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_SEEN, CREATURE_TYPE_IS_ALIVE, TRUE)) ||
            GetIsObjectValid(GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_HEARD, CREATURE_TYPE_IS_ALIVE, TRUE)) ||
            !GetIsObjectValid(oFollow) || GetIsDead(oFollow))
            {
                DeleteLocalObject(OBJECT_SELF,AI_TO_FOLLOW);
                ResetCurrentAction();
                DetermineCombatRound();
            }
            else
            {
                ActionForceFollowObject(oFollow, f3);
                return TRUE;
            }
        }
        break;
...
int GetAreaWidth(object oArea)
{
  int nX = GetLocalInt(GetModule(),"#WIDTH_"+ObjectToString(oArea));
  if( nX == 0)
  {
    int nY = 0; int nColor;
    for (nX = 0; nX < 32; ++nX)
    {
      ...
      if (nColor < 0 || nColor > 32)
      {
        SetLocalInt(GetModule(),"#WIDTH_"+ObjectToString(oArea), nX);
        return(nX);
      }
    }
    SetLocalInt(GetModule(),"#WIDTH_"+ObjectToString(oArea), 32);
    return 32;
  }
  else
    return nX;
...
int GetAreaHeight(object oArea)
{
  int nY = GetLocalInt(GetModule(),"#HEIGHT_"+ObjectToString(oArea));
  if( nY == 0)
  {
    int nX = 0; int nColor;
    for (nY=0; nY<32; ++nY)
    {
      ...
      if (nColor < 0 || nColor > 32)
      {
        SetLocalInt(GetModule(),"#HEIGHT_"+ObjectToString(oArea),nY);
        return(nY);
      }
    }
    SetLocalInt(GetModule(),"#HEIGHT_"+ObjectToString(oArea),32);
    return 32;
  }
  else
    return nY;
...
void SetNumberOfSpellAndAbility()
{
    if(GetLocalInt(OBJECT_SELF,AI_TIMER_SET_NUMBER_S_A)) return;
    SetLocalInt(OBJECT_SELF,AI_TIMER_SET_NUMBER_S_A,TRUE);
    DelayCommand(f30,DeleteLocalInt(OBJECT_SELF,AI_TIMER_SET_NUMBER_S_A));

    int iCnt,iSpell,iAbility;
    // spell count
    for(iCnt=0;iCnt<=6;iCnt++)
    {
        if(GetHasSpell(iCnt))iSpell++;
    }
    ...
    if(GetHasSpell(107))iSpell++;
    if(GetHasSpell(110))iSpell++;
    if(GetHasSpell(111))iSpell++;
    for(iCnt=113;iCnt<=135;iCnt++)
    {
        if(GetHasSpell(iCnt))iSpell++;
    }
    if(GetHasSpell(137))iSpell++;
    ...
    if(GetHasSpell(485))iSpell++;
    if(GetHasSpell(486))iSpell++;
    ...
    if(GetHasSpell(569))iSpell++;
    for(iCnt=636;iCnt<=640;iCnt++)
    {
        if(GetHasSpell(iCnt))iSpell++;
    }
    if(GetHasSpell(695))iSpell++;


    // ability count
    for(iCnt=205;iCnt<=235;iCnt++)
    {
        if(GetHasSpell(iCnt))iAbility++;
    }
    ...
    if(GetHasSpell(686))iAbility++;
    if(GetHasSpell(690))iAbility++;
    for(iCnt=710;iCnt<=715;iCnt++)
    {
        if(GetHasSpell(iCnt))iAbility++;
    }
    if(GetHasSpell(727))iAbility++;
    if(GetHasSpell(731))iAbility++;
    if(GetHasSpell(741))iAbility++;
    if(GetHasSpell(757))iAbility++;
    if(GetHasSpell(763))iAbility++;
    for(iCnt=769;iCnt<=770;iCnt++)
    {
        if(GetHasSpell(iCnt))iAbility++;
    }
    ...
    if(GetHasSpell(789))iAbility++;
    for(iCnt=799;iCnt<=801;iCnt++)
    {
        if(GetHasSpell(iCnt))iAbility++;
    }
    if(GetHasSpell(803))iAbility++;

    SetLocalInt(OBJECT_SELF,AI_NUMBER_OF_SPELLS,iSpell);
    SetLocalInt(OBJECT_SELF,AI_NUMBER_OF_ABILITIES,iAbility);
}
...
/*            if(GetLocalInt(OBJECT_SELF,RULE_VERSION)!=i3)
            {
                sType=GetLocalString(OBJECT_SELF,AI_FAMILIAR_TYPE);
                if(sType=="")
                {
                    switch(Random(11))
                    {
                    ...
                    SetLocalString(OBJECT_SELF,AI_FAMILIAR_TYPE,sType);
                }
                nLevel=GetLevelByClass(CLASS_TYPE_SORCERER);
                if(GetLevelByClass(CLASS_TYPE_WIZARD)>nLevel)nLevel=GetLevelByClass(CLASS_TYPE_WIZARD);
                if(nLevel<i10)
                {
                    sType+="0";
                }
                sType+=IntToString(nLevel);

                object oFamiliar=CreateObject(OBJECT_TYPE_CREATURE,sType,GetLocation(OBJECT_SELF));
                AddHenchman(OBJECT_SELF,oFamiliar);
                if(nEffect)
                {
                    ApplyEffectAtLocation(DURATION_TYPE_INSTANT,EffectVisualEffect(VFX_IMP_UNSUMMON),GetLocation(oFamiliar));
                }
            }
            else*/
            {
...
/*
            if(GetLocalInt(OBJECT_SELF,RULE_VERSION)!=i3)
            {
                sType=GetLocalString(OBJECT_SELF,AI_COMPANION_TYPE);
                if(sType=="")
                {
                    switch(Random(9))
                    {
                    ...
                    SetLocalString(OBJECT_SELF,AI_COMPANION_TYPE,sType);
                }
                nLevel=GetLevelByClass(CLASS_TYPE_DRUID);
                if(GetLevelByClass(CLASS_TYPE_RANGER)>nLevel)nLevel=GetLevelByClass(CLASS_TYPE_RANGER);
                if(nLevel<i10)
                {
                    sType+="0";
                }
