            ...
            if(iInt >= i7)
            {
                ...
                if(
                    ...
                    && (
                        ...
                        || GetItemPossessor(GetObjectByTag(GetLockKeyTag(oBlocker))) == OBJECT_SELF
                    )
                )
                {
                ...
                // Si la puerta esta con llave, no tiene trampa, y la criatura fue generada RandomSpawn, abrir la puerta como si nu tubiera llave a no ser que la puerta tenga la propiedad 'noEsAbriblePorCriaturasAdeptasAlRS'.
                if(
                    GetLocked(oBlocker)
                    && !GetIsTrapped(oBlocker)
                    && GetLocalInt( OBJECT_SELF, RS_nivelPremio_LN ) >0
                    && !GetLocalInt(oBlocker, RS_puertaNoEsAbriblePorCriaturasGeneradasPorElRandoSpawn_PN )
                    //&& GetIsDoorActionPossible(oBlocker, DOOR_ACTION_OPEN)
                    && (
                        !GetLockKeyRequired(oBlocker)
                        || GetItemPossessor(GetObjectByTag(GetLockKeyTag(oBlocker))) == OBJECT_SELF
