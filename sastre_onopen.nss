////////////
// Evento OnOpened para el merchant Sastre
// Author: Inquisidor
//
...
const int Store_DESIRED_ITEMS_QUANTITY = 18;
...
void Store_generateItems( int actualTime, int expectedItemsQuality, int lacking ) {
    ...
    while( --lacking >= 0 ) {
        ...
        switch( Random(6) ) {
            case 0:
                item = IPS_Item_generateCloack( OBJECT_SELF, quality, TRUE );
                break;
            case 1:
                item = IPS_Item_generateBracer( OBJECT_SELF, quality, TRUE );
                break;
            case 2:
                item = IPS_Item_generateBelt( OBJECT_SELF, quality, TRUE );
                break;
            case 3:
                item = IPS_Item_generateBoots( OBJECT_SELF, quality, TRUE );
                break;
            default:
                item = IPS_Item_generateArmor( OBJECT_SELF, quality, ROBE_ARMOR_DESCRIPTORS_ARRAY, TRUE );
                break;
        }
