...
void main()
{
    ...
    int dado = d20();
    int ataque = GetBaseAttackBonus(OBJECT_SELF);
    int destreza = GetAbilityModifier(ABILITY_DEXTERITY);
    int targetAC = GetAC(oTarget);

    //string mensaje = GetName(OBJECT_SELF)+" te lanza una roca! Tirada de ataque: "+IntToString(dado)+" + "+IntToString(ataque+destreza)+" = "+IntToString(dado+destreza+ataque)+" vs. AC "+IntToString(targetAC)+": ";
    location lImpact;
    if (dado+ataque+destreza >= targetAC)
    {
        //mensaje += "acierta!";
        lImpact = GetSpellTargetLocation();
    }
    else
    {
        //mensaje += "erra!";
        float fDist = IntToFloat(Random(20)/10) + RADIUS_SIZE_SMALL;
        lImpact = GetRandomLocation(GetArea(oTarget), oTarget, fDist);
    }
