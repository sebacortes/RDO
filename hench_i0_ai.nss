...
int HenchBashDoorCheck(object oIntruder, int bPolymorphed);



void SetEnemyLocation(object oEnemy)
{
    SetLocalInt(OBJECT_SELF, "LastSeenOrHeard", TRUE);

    location enemyLocation = GetLocation(oEnemy);
    SetLocalLocation(OBJECT_SELF, "LastSeenOrHeard", enemyLocation);
    ...
    object oInvisTarget = GetLocalObject(OBJECT_SELF, "LastSeenOrHeard");
    if (GetIsObjectValid(oInvisTarget))
    {
        DestroyObject(oInvisTarget);
        DeleteLocalObject(OBJECT_SELF, "LastSeenOrHeard");
    }

    oInvisTarget = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", enemyLocation);
    SetLocalObject(OBJECT_SELF, "LastSeenOrHeard", oInvisTarget);
}


void ClearEnemyLocation()
{
    DeleteLocalInt(OBJECT_SELF, "LastSeenOrHeard");
    DeleteLocalLocation(OBJECT_SELF, "LastSeenOrHeard");
...
void MoveToLastSeenOrHeard(int bDoSearch = TRUE)
{
    location moveToLoc = GetLocalLocation(OBJECT_SELF, "LastSeenOrHeard");
...
//    Jug_Debug(GetName(OBJECT_SELF) + " det com round called with " + GetName(oIntruder) + " force = " + IntToString(bForceInterrupt));
    ...
    SetLocalObject(OBJECT_SELF, "AIIntruderObj", oIntruder);
    SetLocalInt(OBJECT_SELF, "AIIntruderForce", bForceInterrupt);

    if (bForceInterrupt)
    {
        SetLocalInt(OBJECT_SELF, "AIIntruder", 1);
        ExecuteScript(sAIScript, OBJECT_SELF);
        return;
    }

    // check if we have to actually determine to rerun ai
    int iAIIntruderLevel = GetLocalInt(OBJECT_SELF, "AIIntruder");

    if (iAIIntruderLevel == 0)
    {
    ...
    if (!GetIsObjectValid(oLastTarget))
    {
        ...
        // unheard monster
        if (!GetLocalInt(OBJECT_SELF, "LastSeenOrHeard") || GetObjectSeen(oIntruder) || GetObjectHeard(oIntruder))
        {
            ExecuteScript(sAIScript, OBJECT_SELF);
        }
    }
...
int HenchTalentAdvancedBuff(float fDistance)
{
    ...
    if(GetIsObjectValid(oPC) && GetDistanceToObject(oPC) <= fDistance)
    {
        if(!GetIsFighting(OBJECT_SELF))
        {
            ClearAllActions();
            //Combat Protections
            if(CU_GetHasSpell(SPELL_PREMONITION))
            {
                CU_ActionCastSpellAtObject(SPELL_PREMONITION, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_GREATER_STONESKIN))
            {
                CU_ActionCastSpellAtObject(SPELL_GREATER_STONESKIN, OBJECT_SELF, METAMAGIC_NONE, 0, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_STONESKIN))
            {
                CU_ActionCastSpellAtObject(SPELL_STONESKIN, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            //Visage Protections
            if(CU_GetHasSpell(SPELL_SHADOW_SHIELD))
            {
                CU_ActionCastSpellAtObject(SPELL_SHADOW_SHIELD, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_ETHEREAL_VISAGE))
            {
                CU_ActionCastSpellAtObject(SPELL_ETHEREAL_VISAGE, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_GHOSTLY_VISAGE))
            {
                CU_ActionCastSpellAtObject(SPELL_GHOSTLY_VISAGE, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            //Mantle Protections
            if(CU_GetHasSpell(SPELL_GREATER_SPELL_MANTLE))
            {
                CU_ActionCastSpellAtObject(SPELL_GREATER_SPELL_MANTLE, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SPELL_MANTLE))
            {
                CU_ActionCastSpellAtObject(SPELL_SPELL_MANTLE, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_LESSER_SPELL_BREACH))
            {
                CU_ActionCastSpellAtObject(SPELL_LESSER_SPELL_BREACH, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            // Globes
            if(CU_GetHasSpell(SPELL_GLOBE_OF_INVULNERABILITY))
            {
                CU_ActionCastSpellAtObject(SPELL_GLOBE_OF_INVULNERABILITY, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_MINOR_GLOBE_OF_INVULNERABILITY))
            {
                CU_ActionCastSpellAtObject(SPELL_MINOR_GLOBE_OF_INVULNERABILITY, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }

            //Misc Protections
            if(CU_GetHasSpell(SPELL_ELEMENTAL_SHIELD))
            {
                CU_ActionCastSpellAtObject(SPELL_ELEMENTAL_SHIELD, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if (CU_GetHasSpell(SPELL_MESTILS_ACID_SHEATH)&& !GetHasSpellEffect(SPELL_MESTILS_ACID_SHEATH))
            {
                CU_ActionCastSpellAtObject(SPELL_MESTILS_ACID_SHEATH, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if (CU_GetHasSpell(SPELL_DEATH_ARMOR)&& !GetHasSpellEffect(SPELL_DEATH_ARMOR))
            {
                CU_ActionCastSpellAtObject(SPELL_DEATH_ARMOR, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }

            //Elemental Protections
            if(CU_GetHasSpell(SPELL_PROTECTION_FROM_ELEMENTS))
            {
                CU_ActionCastSpellAtObject(SPELL_PROTECTION_FROM_ELEMENTS, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_RESIST_ELEMENTS))
            {
                CU_ActionCastSpellAtObject(SPELL_RESIST_ELEMENTS, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_ENDURE_ELEMENTS))
            {
                CU_ActionCastSpellAtObject(SPELL_ENDURE_ELEMENTS, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }

            //Mental Protections
            if(CU_GetHasSpell(SPELL_MIND_BLANK))
            {
                CU_ActionCastSpellAtObject(SPELL_MIND_BLANK, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_LESSER_MIND_BLANK))
            {
                CU_ActionCastSpellAtObject(SPELL_LESSER_MIND_BLANK, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_CLARITY))
            {
                CU_ActionCastSpellAtObject(SPELL_CLARITY, OBJECT_SELF, METAMAGIC_NONE, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            //Summon Ally
            // TODO add gate!!!!
            if(CU_GetHasSpell(SPELL_BLACK_BLADE_OF_DISASTER))
            {
                CU_ActionCastSpellAtLocation(SPELL_BLACK_BLADE_OF_DISASTER, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_ELEMENTAL_SWARM))
            {
                CU_ActionCastSpellAtLocation(SPELL_ELEMENTAL_SWARM, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_IX))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_IX, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_CREATE_GREATER_UNDEAD))
            {
                CU_ActionCastSpellAtLocation(SPELL_CREATE_GREATER_UNDEAD, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_GREATER_PLANAR_BINDING))
            {
                CU_ActionCastSpellAtLocation(SPELL_GREATER_PLANAR_BINDING, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_VIII))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_VIII, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_MORDENKAINENS_SWORD))
            {
                CU_ActionCastSpellAtLocation(SPELL_MORDENKAINENS_SWORD, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_VII))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_VII, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_CREATE_UNDEAD))
            {
                CU_ActionCastSpellAtLocation(SPELL_CREATE_UNDEAD, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_PLANAR_BINDING))
            {
                CU_ActionCastSpellAtLocation(SPELL_PLANAR_BINDING, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_VI))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_VI, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_V))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_V, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELLABILITY_SUMMON_SLAAD))
            {
                CU_ActionCastSpellAtLocation(SPELLABILITY_SUMMON_SLAAD, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELLABILITY_SUMMON_TANARRI))
            {
                CU_ActionCastSpellAtLocation(SPELLABILITY_SUMMON_TANARRI, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELLABILITY_SUMMON_MEPHIT))
            {
                CU_ActionCastSpellAtLocation(SPELLABILITY_SUMMON_MEPHIT, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELLABILITY_SUMMON_CELESTIAL))
            {
                CU_ActionCastSpellAtLocation(SPELLABILITY_SUMMON_CELESTIAL, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_ANIMATE_DEAD))
            {
                CU_ActionCastSpellAtLocation(SPELL_ANIMATE_DEAD, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_IV))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_IV, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_III))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_III, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(CU_GetHasSpell(SPELL_SUMMON_CREATURE_II))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_II, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            else if(GetHasSpell(SPELL_SUMMON_CREATURE_I))
            {
                CU_ActionCastSpellAtLocation(SPELL_SUMMON_CREATURE_I, GetLocation(OBJECT_SELF), METAMAGIC_NONE, FALSE, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
            }
            return TRUE;
        }
    }
