...
int HenchFindSpecificSpell(int iSpell, int iCategory, int iCR)
{
    ...
    talent tCurrent = CU_GetCreatureTalentBest(iCategory, iCR);
    while (nTry < 5 && GetIsTalentValid(tCurrent))
    {
        ...
        nTry ++;
        tCurrent = CU_GetCreatureTalentRandom(iCategory);
    }
    return FALSE;
}
...
{    
    if (CU_GetHasSpell(SPELL_MAGIC_MISSILE))
    {        
        if (bDoMoveAway && GetDistanceToObject(oTarget) < 5.0)
        {            
             ActionMoveAwayFromObject(oTarget, FALSE, 5.0);
        }        
        CU_ActionCastSpellAtObject(SPELL_MAGIC_MISSILE, oTarget);
        return TRUE;
    }    
    return FALSE;
}
...
{    
    ...
    if (CU_GetHasSpell(SPELL_FIND_TRAPS) && GetTrapDisarmable(oTrap) && GetLocalInt(oTrap, "NW_L_IATTEMPTEDTODISARMNOWORK") == 0)
    {
        CU_ActionCastSpellAtObject(SPELL_FIND_TRAPS, oTrap);
...
void DoOpenLock(object oLocked)
{
        ...
        GetIsPlaceableObjectActionPossible(oLocked, PLACEABLE_ACTION_KNOCK))
    {
        ...
        CU_ActionCastSpellAtObject(SPELL_KNOCK, oLocked);
        ActionWait(1.0);
        return;
    }
