...
************************* [Targeting] *****************************************/
   ...
   // AI_CreateRandomStats(0, 3, 6);
        // Create (Effect-applied) random statistics.
  //  AI_CreateRandomOther(0, 20, 0, 5, 0, 5, 0, 5, 0, 5,AC_DODGE_BONUS);
...
************************* [Combat - Fighters] *********************************/
    ...
    if(GetAbilityScore(OBJECT_SELF, ABILITY_DEXTERITY, FALSE) <= GetAbilityScore(OBJECT_SELF, ABILITY_STRENGTH, FALSE))
    {
...
/*
    int dif2 = GetLocalInt(GetArea(OBJECT_SELF), "dif");
    if(dif2 > 5)
    {
    AI_CreateRandomStats(0, dif2-3, 6);
        // Create (Effect-applied) random statistics.
    AI_CreateRandomOther(0, dif2-2*5, 0, dif2-2, 0, dif2-2, 0, dif2-2, 0, dif2-2);
    }
*/
        // Create (Effect-applied) random HP, saves, AC.
...
//    if(GetSkillRank(SKILL_HIDE, OBJECT_SELF) > 10 || GetHasSpell(SPELL_INVISIBILITY ,OBJECT_SELF) || GetHasSpell(SPELL_INVISIBILITY_SPHERE ,OBJECT_SELF) || GetHasSpell(SPELL_IMPROVED_INVISIBILITY ,OBJECT_SELF))
//        PerceptionSys_playerHandler();

    // Aplica diferentes efectos de acuerdo a variables locales o propiedades de la criatura (fantasma, undead, etc)
    AI_SetUpMiscEffects(OBJECT_SELF);

     //DMFI CODE ADDITIONS BEGIN HEREERE
