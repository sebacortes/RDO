...
const int GlobalEffectImmunityDrain               = 0x01000000;// many use?
const int GlobalEffectTensersTransformation       = 0x02000000;// Tenser's Transformation
const int GlobalEffectImmunityNecromancy          = 0x04000000;// many use?
const int GlobalEffectFreedomOfMovement           = 0x08000000;// for bigby check
...
void AI_SetEffectsOnTarget(object oTarget = OBJECT_SELF)
{
    ...
    {
        if(GetLocalInt(oTarget, AI_TIMER_EFFECT_SET))return;
        SetLocalInt(oTarget, AI_TIMER_EFFECT_SET, TRUE);
        // Just set effects every round, not every possible action (2 with haste)
        DelayCommand(f2, DeleteLocalInt(oTarget, AI_TIMER_EFFECT_SET));
    }
/*
    if(!GetLocalInt(OBJECT_SELF, AI_JASPERRES_EFFECT_SET))
    {
        if(GetLocalInt(oTarget, AI_TIMER_EFFECT_SET))return;
        SetLocalInt(oTarget, AI_TIMER_EFFECT_SET, TRUE);
        // Just set effects every round, not every possible action (2 with haste)
...
*/
    ...
    int iEffect, iEffectAbilityDecrease, iSpellID,iCnt;
    object oSlot;
    itemproperty ip;

    // add item property check here. lack...
    for(iCnt=INVENTORY_SLOT_HEAD;iCnt<=INVENTORY_SLOT_CARMOUR;iCnt++)
    {
        oSlot=GetItemInSlot(iCnt,oTarget);
            ...
            while(GetIsItemPropertyValid(ip))
            {
                switch(GetItemPropertyType(ip))
                {
                    ...
                    case ITEM_PROPERTY_IMMUNITY_MISCELLANEOUS:
                        if(GetItemPropertySubType(ip)==IP_CONST_IMMUNITYMISC_DEATH_MAGIC)
                        {
                            AI_SetWeHaveSpellsEffect(GlobalHasDeathWardSpell);
                        }
                        else if(GetItemPropertySubType(ip)==IP_CONST_IMMUNITYMISC_MINDSPELLS)
                        {
                            AI_SetWeHaveSpellsEffect(GlobalHasMindResistanceProtections);
                        }
                        else if(GetItemPropertySubType(ip)==IP_CONST_IMMUNITYMISC_LEVEL_ABIL_DRAIN)
                        {
                            AI_SetWeHaveEffect(GlobalEffectImmunityDrain);
                        }
                        break;
                    case ITEM_PROPERTY_IMMUNITY_SPELL_SCHOOL:
                        if(GetItemPropertySubType(ip)==IP_CONST_SPELLSCHOOL_NECROMANCY)
                        {
                            AI_SetWeHaveEffect(GlobalEffectImmunityNecromancy);
                        }
                        break;
                    case ITEM_PROPERTY_FREEDOM_OF_MOVEMENT:
                        AI_SetWeHaveEffect(GlobalEffectFreedomOfMovement);
                        break;
                    case ITEM_PROPERTY_IMMUNITY_SPELLS_BY_LEVEL:
                        SetLocalInt(oTarget,AI_SPELL_IMMUNE_LEVEL,GetItemPropertyCostTableValue(ip));
                        break;
                    case ITEM_PROPERTY_ONHITCASTSPELL:
                        switch(GetItemPropertySubType(ip))
                        {
                            case IP_CONST_ONHIT_CASTSPELL_DISPEL_MAGIC:
                            case IP_CONST_ONHIT_CASTSPELL_GREATER_DISPELLING:
                            case IP_CONST_ONHIT_CASTSPELL_LESSER_DISPEL:
                            case IP_CONST_ONHIT_DISPELMAGIC:
                            case IP_CONST_ONHIT_GREATERDISPEL:
                            case IP_CONST_ONHIT_LESSERDISPEL:
                                SetSpawnInCondition(AI_FLAG_OTHER_COMBAT_DISPEL_WITH_MELEE, AI_OTHER_COMBAT_MASTER);
                        }
                        break;
    ...
    while(GetIsEffectValid(eCheck))
    {
        ...
        switch(iEffect)
        {
            ...
            case EFFECT_TYPE_CONFUSED: // added by modifier
            case EFFECT_TYPE_DISAPPEARAPPEAR:// Added for dragon flying
            case EFFECT_TYPE_DOMINATED: // added in modify.
...
//          bug.. effect of greater sanctuary is EFFECT_TYPE_SANCTUARY
                ...
                if(iSpellID != iM1)
                {
                    switch(iSpellID)
                    {
                        ...
                        case SPELL_TENSERS_TRANSFORMATION:
                            AI_SetWeHaveEffect(GlobalEffectTensersTransformation);
                        break;
                    }
                }
            }
