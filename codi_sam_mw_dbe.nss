/*
    Increase masterwork weapons to give +d6 electrical damage.
*/
...
void main()
{
    ...
    if (GetItemHasItemProperty(oWeapon, ITEM_PROPERTY_DAMAGE_BONUS))
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            ...
            {
                if(GetItemPropertySubType(ip) == IP_CONST_DAMAGETYPE_ELECTRICAL)
                {
                    ...
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_ELECTRICAL,iNew), oWeapon);
                    iBreak = TRUE;
                }
            }
