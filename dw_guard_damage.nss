//::///////////////////////////////////////////////
//:: FileName dw_guard_damage
//:://////////////////////////////////////////////
/*
    When the object this script is attached to is damaged, the script finds the nearest
    creature of the same faction and gets them to attack the damager.
*/
//:://////////////////////////////////////////////
...
//When the above checks have finished, the player will be granted 2 chances to stop his/her behaviour or face the consequences.
        if (GetLocalInt(OBJECT_SELF, "GenericDoorGuard1") == 3 )
        {
        AssignCommand(DoorGuard0, SpeakString("FOOL!", TALKVOLUME_TALK));
        }

        if (GetLocalInt(OBJECT_SELF, "GenericDoorGuard1") == 2 )
        {
        AssignCommand(DoorGuard0, SpeakString("FOOL!", TALKVOLUME_TALK));
        ...
        if (GetLocalInt(OBJECT_SELF, "GenericDoorGuard1") == 1 )
        {
        AssignCommand(DoorGuard0, SpeakString("I WON'T WARN YOU AGAIN!!!", TALKVOLUME_TALK));
        SetLocalInt(OBJECT_SELF, "GenericDoorGuard1", 2);
        AdjustAlignment(Player1, ALIGNMENT_CHAOTIC, 1);
        }

        if (GetLocalInt(OBJECT_SELF, "GenericDoorGuard1") == 0 )
        {
        AssignCommand(DoorGuard0, SpeakString("Stop that at once!", TALKVOLUME_TALK));
        SetLocalInt(OBJECT_SELF, "GenericDoorGuard1", 1);
        AdjustAlignment(Player1, ALIGNMENT_CHAOTIC, 1);
        }
