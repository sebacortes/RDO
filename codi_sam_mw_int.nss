...
void main()
{
    ...
    if (TRUE)
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            ...
            {
                //SpeakString("Ability Strength: " + IntToString(ABILITY_INTELLIGENCE));
                ...
                if(GetItemPropertySubType(ip) == ABILITY_INTELLIGENCE)
                {
                    iValue = GetItemPropertyCostTableValue(ip);
                    RemoveItemProperty(oWeapon, ip);
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyAbilityBonus(ABILITY_INTELLIGENCE, iValue + 1), oWeapon);
                    iBreak = TRUE;
                }
            }
        ...
        if(!iBreak)
        {
            ip = ItemPropertyAbilityBonus(ABILITY_INTELLIGENCE, 1);
