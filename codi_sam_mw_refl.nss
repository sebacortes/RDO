/*
    Increase masterwork weapons reflex save.
*/
    ...
    if (TRUE)
    {
        ...
        while (GetIsItemPropertyValid(ip) && !iBreak)
        {
            if(GetItemPropertyType(ip) == ITEM_PROPERTY_SAVING_THROW_BONUS_SPECIFIC && GetItemPropertyDurationType(ip) == DURATION_TYPE_PERMANENT)
            {
                ...
                if(GetItemPropertySubType(ip) == SAVING_THROW_REFLEX)
                {
                    iValue = GetItemPropertyCostTableValue(ip);
                    RemoveItemProperty(oWeapon, ip);
                    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyBonusSavingThrow(SAVING_THROW_REFLEX, iValue + 2), oWeapon);
                    iBreak = TRUE;
                }
            }
        ...
        if(!iBreak)
        {
            ip = ItemPropertyBonusSavingThrow(SAVING_THROW_REFLEX, 2);
