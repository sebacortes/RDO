...
const int     X2_CI_BREWPOTION_COSTMODIFIER   = 10;                     // gp Brew Potion XPCost Modifier
...
const int     X2_CI_SCRIBESCROLL_COSTMODIFIER   = 5;                 // Scribescroll Cost Modifier
...
// sirve para marcar los items creados por el craft para no ser destruidos por el control de items ilegales
const string RdO_LEGAL_CRAFT_ITEM_MARK = "RdO_LEGAL_CRAFT_ITEM_MARK";
...
object CICraftBrewPotion(object oCreator, int nSpellID )
{
    ...
    if (nPropID != -1)
    {
        ...
        // Agregado por Vermis - Pone nombre a las pociones
        string sSpellName = GetStringByStrRef(StringToInt(Get2DAString("spells","Name",nSpellID))); // se podria modificar por la rutina del prc que cachea el nombre de los conjuros
        SetName(oTarget,sSpellName);
        // Aqui se podria agregar codigo para que la descripcion del objeto conteniera el nombre del pj que la creo.
        // SetDescription(oTarget,GetDescription(oTarget)+"\n"+GetName(oCreator)) el \n para emular una linea de enter //
        AddItemProperty(DURATION_TYPE_PERMANENT,ipProp,oTarget);
        SetLocalInt(oTarget, RdO_LEGAL_CRAFT_ITEM_MARK, TRUE);
    }
    return oTarget;
}
