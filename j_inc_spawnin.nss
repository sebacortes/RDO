...
// Functions:
/******************************************************************************/
// set misc effects in spawning.
void AI_SetUpMiscEffects(object oTarget=OBJECT_SELF);
...
//  Determines combat round, if not fighting
    ...
    // modified - need to cast dispel to melee target!
    SetListenPattern(OBJECT_SELF, DISPEL_THIS_TARGET, i9);
...
void AI_AdvancedAuras()
{
    if(GetHasSpell(SPELL_ETHEREALNESS)) return;
    if(GetSpawnInCondition(AI_VALID_TALENT_PERSISTENT_AREA_OF_EFFECT, AI_VALID_SPELLS))
    {
        ...
        AI_ActivateAura(SPELLABILITY_TROGLODYTE_STENCH);
    }
}
...
void AI_SetUpSpells()
{
    ...
    ***************************************************************************/
    ...
    // Valid?
    if(GetIsTalentValid(tCheck) || GetHasSpell(771))
    {
        // Then set we have it
...
//       GetHasSpell(SPELL_AMPLIFY) ||
    ...
    if(GetHasSpell(SPELL_GREATER_RESTORATION) || GetHasSpell(SPELL_FREEDOM_OF_MOVEMENT) ||
       ...
       GetHasSpell(SPELL_STONE_TO_FLESH) || GetHasSpell(AI_SPELLABILITY_BEHOLDER_MAGIC_CONE) ||
       GetHasSpell(SPELL_LESSER_DISPEL) || GetHasSpell(SPELL_DISPEL_MAGIC) ||
       GetHasSpell(SPELL_GREATER_DISPELLING)|| GetHasSpell(SPELL_MORDENKAINENS_DISJUNCTION))
    {
...
void AI_SetUpMiscEffects(object oTarget=OBJECT_SELF)
{
    ...
    if(GetLocalInt(oTarget,"GHOST_EFFECT"))
    {
        effect eTemp;
        eTemp=SupernaturalEffect(EffectLinkEffects(EffectVisualEffect(VFX_DUR_GHOSTLY_VISAGE),EffectCutsceneGhost()));
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,eTemp,oTarget);
    }

    if(GetLocalInt(oTarget,"ICESKIN_EFFECT"))
    {
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectVisualEffect(VFX_DUR_ICESKIN)),oTarget);
    }

    if(GetLocalInt(oTarget,"ULTRAVISION_EFFECT"))
    {
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectUltravision()),oTarget);
    }

    if(GetLocalInt(oTarget,"CONCEAL_EFFECT")>0)
    {
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectConcealment(GetLocalInt(oTarget,"CONCEAL_EFFECT"))),oTarget);
    }

    if(GetLocalInt(oTarget,"SPELLRESISTANCE_EFFECT")>0)
    {
        ApplyEffectToObject(DURATION_TYPE_PERMANENT,SupernaturalEffect(EffectSpellResistanceIncrease(GetLocalInt(oTarget,"SPELLRESISTANCE_EFFECT"))),oTarget);
    }
