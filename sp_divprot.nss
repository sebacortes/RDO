...
void main()
{
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_ALLALLIES, OBJECT_SELF))
        {
            ...
            if (GetLocalInt(OBJECT_SELF, "sp_divprot")!=TRUE) {
                DelayCommand(fDelay, SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, eCurse, oTarget, fDuration,TRUE,-1,nCasterLvl));
                SetLocalInt(OBJECT_SELF, "sp_divprot", TRUE);
                DelayCommand(fDuration, DeleteLocalInt(OBJECT_SELF, "sp_divprot"));
            } else
                SendMessageToPC(OBJECT_SELF, "Este conjuro no apila consigo mismo.");
