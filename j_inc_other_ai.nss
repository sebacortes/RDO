...
void RespondToShout(object oShouter, int nShoutIndex)
{
    ...
    else if(nShoutIndex == i9)// "DISPEL_THIS_TARGET"
    {
        SetLocalTimer(AI_TIMER_SHOUT_IGNORE_ANYTHING_SAID, f6);
        object oTmp=GetLocalObject(oShouter,AI_SAVED_DISPEL_TARGET);
        SetLocalObject(OBJECT_SELF,AI_SAVED_DISPEL_TARGET,oTmp);
        if(GetObjectType(oTmp)==OBJECT_TYPE_AREA_OF_EFFECT)
        {
            SetLocalInt(OBJECT_SELF,AI_SAVED_DISPEL_TARGET,GetLocalInt(oShouter,AI_SAVED_DISPEL_TARGET));
        }
    }
...
//           nShoutIndex == i4 || // "Help my friend"
           ...
           nShoutIndex == i7)
        {
            ...
            else // Else the enemy is not seen
            {
                ...
                SetLocalTimer(AI_TIMER_SHOUT_IGNORE_ANYTHING_SAID, f6);
                // set target
                SetLocalObject(OBJECT_SELF,AI_TO_ATTACK,oIntruder);
        ...
        else if(nShoutIndex == i4)// "Help my friend"
        {
            ...
            else
            {
                ...
                GetIsObjectValid(GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_HEARD, CREATURE_TYPE_IS_ALIVE, TRUE))))
                {
                    ...
                    // set intruder for detect check
                    SetLocalObject(OBJECT_SELF,AI_TO_FOLLOW,oShouter);
                    ...
                    ClearAllActions();
                    SetCurrentAction(AI_SPECIAL_ACTIONS_FOLLOW_ALLY);
                    ActionForceFollowObject(oShouter, f3);
//                    ActionDoCommand(DetermineCombatRound());
//                    DetermineCombatRound(oIntruder);
                }
