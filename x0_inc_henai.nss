...
// Si bkGetBehavior(BK_CURRENT_AI_MODE) != BK_AI_MODE_RUN_AWAY
// y esta a mas de BK_FOLLOW_THRESHOLD metros de su master, se le ordea que lo siga.
// Devuelve TRUE si se le dio la orden de seguir, o bkGetBehavior(BK_NEVERFIGHT) == TRUE
// Nota: si la varialbe Party esta en TRUE, no hace nada y devuelve TRUE.
...
*/
    ...
    if (GetIsObjectValid(oIntruder) || GetIsObjectValid(oNearestTarget))
    {
		...
		// QUITADO: porque no esta en la version original de bioware
        //ExecuteScript("aihench", OBJECT_SELF);
    }
}
...
/*    int iSurrendered = GetLocalInt(OBJECT_SELF,"Generic_Surrender");
    ...
    case ASSOCIATE_COMMAND_INVENTORY:
		...
		// QUITADO: para dejar que siempre se pueda acceder al inventario
    ...
    case ASSOCIATE_COMMAND_ATTACKNEAREST:
		...
		// QUITADO: porque no esta en la version original de bioware
        //SetLocalInt(OBJECT_SELF, "Seguir", FALSE);
        //SetLocalObject(OBJECT_SELF, "blanco", OBJECT_INVALID);
        //ExecuteScript("aihench", OBJECT_SELF);
    ...
    case ASSOCIATE_COMMAND_FOLLOWMASTER:
		...
		// QUITADO: porque no esta en la version original de bioware
        //SetLocalInt(OBJECT_SELF, "Seguir", TRUE);
...
{
	// QUITADO: porque no esta en la version original de bioware
    //if(GetLocalInt(OBJECT_SELF, "Seguir") == TRUE)
    //    return TRUE;
...
//    SetCommandable(FALSE);
		// QUITADO: por alguna razón que desconozco fue quitado, asi que lo dejo asi
        //if (d10() > 7)
        //{
        //    if (iAmHenchman || iAmFamiliar)
        //        VoiceLookHere();
