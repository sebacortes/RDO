...
void main()
{
    ...
    else
    {
        ...
        for (i = 1; i <= iMax; i++)
        {
            ...
            int nInnateLevel = StringToInt(Get2DAString("spells","Innate",nSpellId-1));
            if(nInnateLevel<5)
            {
            if (nSpellId>0)
            {
                ...
                if(GetHasSpell(nSpellId, OBJECT_SELF))
                {
                ActionCastSpellAtObject(nSpellId, oPC, METAMAGIC_ANY, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
                }
                else
                if(GetSkillRank(SKILL_USE_MAGIC_DEVICE, OBJECT_SELF) > GetSkillRank(SKILL_SPELLCRAFT, OBJECT_SELF))
                {
                    if(GetIsSkillSuccessful(OBJECT_SELF, SKILL_USE_MAGIC_DEVICE, 25+nInnateLevel))
                    {
                    ActionCastSpellAtObject(nSpellId, oPC, METAMAGIC_ANY, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
                    }
                }
                else
                if(GetIsSkillSuccessful(OBJECT_SELF, SKILL_SPELLCRAFT, 15+nInnateLevel))
                {
                ActionCastSpellAtObject(nSpellId, oPC, METAMAGIC_ANY, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
                }
