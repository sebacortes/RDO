////////////
// Evento OnOpened para el merchant Articulosparamago
// Author: Inquisidor
//
...
void Store_generateItems( int actualTime, int expectedItemsQuality, int lacking ) {
    ...
    while( --lacking >= 0 ) {
        ...
        switch( Random(2) ) {
            case 0:
                item = IPS_Item_generateRing( OBJECT_SELF, quality, TRUE );
                break;
            case 1:
                item = IPS_Item_generateNecklace( OBJECT_SELF, quality, TRUE );
                break;
        }
    }
}
