...
// End of Spell Cast Hook
    ...
    int nPenetr = Sol+add_spl_pen(OBJECT_SELF);
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(GetIsFriend(oTarget))
        {
            ...
            SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, ePosLink, oTarget,RoundsToSeconds(nDuration),TRUE,-1,Sol);
        }
        else if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if(!MyPRCResistSpell(OBJECT_SELF, oTarget,nPenetr))
            {
                ...
                SPApplyEffectToObject(DURATION_TYPE_TEMPORARY, eNegLink, oTarget, RoundsToSeconds(nDuration),TRUE,-1,Sol);
            }
        }
